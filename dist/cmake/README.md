# CMake Support for libvoid-vt

The top-level `CMakeLists.txt` wraps the Zig build system so that CMake
projects can consume libvoid-vt without invoking `zig build` manually.
...
FetchContent_Declare(void
    GIT_REPOSITORY https://github.com/ghostty-org/ghostty.git
    GIT_TAG main
)
FetchContent_MakeAvailable(void)

add_executable(myapp main.c)
target_link_libraries(myapp PRIVATE void-vt)
```

This fetches the Void source, builds libvoid-vt via Zig during your
CMake build, and links it into your target. Headers are added to the
include path automatically.

### Using a local checkout

If you already have the Void source checked out, skip the download by
pointing CMake at it:

```shell-session
cmake -B build -DFETCHCONTENT_SOURCE_DIR_VOID=/path/to/void
cmake --build build
```

## Using find_package (install-based)

Build and install libvoid-vt first:
...
```cmake
find_package(void-vt REQUIRED)

add_executable(myapp main.c)
target_link_libraries(myapp PRIVATE void-vt::void-vt)
```

## Cross-compilation

For cross-compiling to a different Zig target triple, use
`void_vt_add_target()` after `FetchContent_MakeAvailable`:

```cmake
FetchContent_MakeAvailable(void)
void_vt_add_target(NAME linux-amd64 ZIG_TARGET x86_64-linux-gnu)

add_executable(myapp main.c)
target_link_libraries(myapp PRIVATE void-vt-static-linux-amd64)
```

### Using zig cc as the C/CXX compiler

When cross-compiling, the host C compiler can't link binaries for the
target platform. `VoidZigCompiler.cmake` provides
`void_zig_compiler()` to set up `zig cc` as the C/CXX compiler for
...
include(cmake/VoidZigCompiler.cmake)
void_zig_compiler(ZIG_TARGET x86_64-linux-gnu)
