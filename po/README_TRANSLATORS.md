# Localizing Void: The Translators' Guide

First of all, thanks for helping us localize Void!
...
Full locale names are more complicated, but Void does not use all parts. [The
...
file called `com.need-singularity.void.pot`. **Do not edit this file.** The
template is generated automatically from Void's code and resources, and is
...
```console
$ msginit -i po/com.need-singularity.void.pot -l X -o "po/X.po"
```
...
Afterwards, you need to update the list of known locales within Void's
...
Simply run `zig build run`. Void uses your system language by default; if
...
On some desktop environments, such as KDE Plasma, Void uses server-side
decorations by default. This hides many strings from the UI, which is
undesirable when viewing your translations. You can force Void to use
...
Localization teams are represented as teams in the Void GitHub organization.
...
```diff
 ...
 /po/com.need-singularity.void.pot @ghostty-org/localization
...
- **Make Void fit in with surrounding applications.**
  ...
  interface guidelines of each platform Void is available for, including the
