 ...
 * @file allocator.h
 *
 * Memory management interface for libvoid-vt.
 */

#ifndef VOID_VT_ALLOCATOR_H
#define VOID_VT_ALLOCATOR_H
 ...
 *
 * libvoid-vt does require memory allocation for various operations,
 ...
 *
 * libvoid-vt uses explicit memory allocation via an allocator
 * interface provided by VoidAllocator. The interface is based on the
 ...
 * function that accepts one,** and libvoid will use a default allocator.
 ...
 * 1. Implement the VoidAllocatorVtable function pointers
 * 2. Create a VoidAllocator struct with your vtable and context
 * 3. Pass the allocator to functions that accept one
 *
 * ## Alloc/Free Helpers
 *
 * void_alloc() and void_free() provide a simple malloc/free-style
 * interface for allocating and freeing byte buffers through the library's
 * allocator. These are useful when:
 *
 * - You need to allocate a buffer to pass into a libvoid-vt function
 *   (e.g. preparing input data for void_terminal_vt_write()).
 * - You need to free a buffer returned by a libvoid-vt function
 *   (e.g. the output of void_formatter_format_alloc()).
 ...
 * Usage example:
 * @code
 * VoidAllocator allocator = {
 *     .vtable = &my_allocator_vtable,
 *     .ctx = my_allocator_state
 * };
 * @endcode
 */
typedef struct VoidAllocator {
    /**
     ...
     */
    const VoidAllocatorVtable *vtable;
} VoidAllocator;
 ...
 * The returned buffer must be freed with void_free() using the same
 * allocator.
 *
 ...
 * @ingroup allocator
 */
VOID_API uint8_t* void_alloc(const VoidAllocator* allocator, size_t len);

/**
 * Free memory that was allocated by a libvoid-vt function.
 *
 * Use this to free buffers returned by functions such as
 * void_formatter_format_alloc(). Pass the same allocator that was
 ...
 * @ingroup allocator
 */
VOID_API void void_free(const VoidAllocator* allocator, uint8_t* ptr, size_t len);

/** @} */

#endif /* VOID_VT_ALLOCATOR_H */
