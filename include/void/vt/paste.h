...
#ifndef VOID_VT_PASTE_H
#define VOID_VT_PASTE_H
 ...
 * ## Basic Usage
 *
 * Use void_paste_is_safe() to check if paste data contains potentially
 * dangerous sequences before sending it to the terminal.
 *
 * Use void_paste_encode() to encode paste data for writing to the pty,
 ...
 */
VOID_API bool void_paste_is_safe(const char* data, size_t len);
 ...
 * VOID_OUT_OF_SPACE and sets the required size in @p out_written.
 ...
 *         is too small
 */
VOID_API VoidResult void_paste_encode(
    char* data,
    size_t data_len,
    bool bracketed,
    char* buf,
