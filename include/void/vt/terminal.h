...
#ifndef VOID_VT_TERMINAL_H
#define VOID_VT_TERMINAL_H
...
#include <void/vt/device.h>
#include <void/vt/modes.h>
#include <void/vt/size_report.h>
#include <void/vt/grid_ref.h>
#include <void/vt/kitty_graphics.h>
#include <void/vt/screen.h>
#include <void/vt/point.h>
 ...
 * to write keyboard input via void_key_encoder_setopt_from_terminal().
 *
 ...
 * ## Effects
 *
 * By default, the terminal sequence processing with void_terminal_vt_write() 
 ...
 * sequences processed during void_terminal_vt_write(). They let the
 ...
 *
 * Each effect is registered with void_terminal_set() using the
 * corresponding `VoidTerminalOption` identifier. A `NULL` value
 * pointer clears the callback and disables the effect.
 *
 * A userdata pointer can be attached via `VOID_TERMINAL_OPT_USERDATA`
 ...
 * void_terminal_vt_write(). Callbacks **must not** call
 * void_terminal_vt_write() on the same terminal (no reentrancy).
 ...
 * |-----------------------------------------|-----------------------------------|-------------------------------------------|
 * | `VOID_TERMINAL_OPT_WRITE_PTY`        | `VoidTerminalWritePtyFn`       | Query responses written back to the pty   |
 * | `VOID_TERMINAL_OPT_BELL`             | `VoidTerminalBellFn`           | BEL character (0x07)                      |
 * | `VOID_TERMINAL_OPT_TITLE_CHANGED`    | `VoidTerminalTitleChangedFn`   | Title change via OSC 0 / OSC 2            |
 * | `VOID_TERMINAL_OPT_ENQUIRY`          | `VoidTerminalEnquiryFn`        | ENQ character (0x05)                      |
 * | `VOID_TERMINAL_OPT_XTVERSION`        | `VoidTerminalXtversionFn`      | XTVERSION query (CSI > q)                 |
 * | `VOID_TERMINAL_OPT_SIZE`             | `VoidTerminalSizeFn`           | XTWINOPS size query (CSI 14/16/18 t)      |
 * | `VOID_TERMINAL_OPT_COLOR_SCHEME`     | `VoidTerminalColorSchemeFn`    | Color scheme query (CSI ? 996 n)          |
 * | `VOID_TERMINAL_OPT_DEVICE_ATTRIBUTES`| `VoidTerminalDeviceAttributesFn`| Device attributes query (CSI c / > c / = c)|
 *
 ...
 * ### Default Colors
 *
 * Use void_terminal_set() with the color options to configure the
 ...
 * |-----------------------------------------|-------------------------|--------------------------------------|
 * | `VOID_TERMINAL_OPT_COLOR_FOREGROUND` | `VoidColorRgb*`      | Default foreground color             |
 * | `VOID_TERMINAL_OPT_COLOR_BACKGROUND` | `VoidColorRgb*`      | Default background color             |
 * | `VOID_TERMINAL_OPT_COLOR_CURSOR`     | `VoidColorRgb*`      | Default cursor color                 |
 * | `VOID_TERMINAL_OPT_COLOR_PALETTE`    | `VoidColorRgb[256]*` | Default 256-color palette            |
 *
 ...
 * ### Reading colors
 *
 * Use void_terminal_get() to read colors. There are two variants for
 ...
 * |---------------------------------------------------|-------------------------|------------------------------------------------|
 * | `VOID_TERMINAL_DATA_COLOR_FOREGROUND`          | `VoidColorRgb*`      | Effective foreground (override or default)      |
 * | `VOID_TERMINAL_DATA_COLOR_BACKGROUND`          | `VoidColorRgb*`      | Effective background (override or default)      |
 * | `VOID_TERMINAL_DATA_COLOR_CURSOR`              | `VoidColorRgb*`      | Effective cursor (override or default)          |
 * | `VOID_TERMINAL_DATA_COLOR_PALETTE`             | `VoidColorRgb[256]*` | Current palette (with any OSC overrides)        |
 * | `VOID_TERMINAL_DATA_COLOR_FOREGROUND_DEFAULT`  | `VoidColorRgb*`      | Default foreground only (ignores OSC override)  |
 * | `VOID_TERMINAL_DATA_COLOR_BACKGROUND_DEFAULT`  | `VoidColorRgb*`      | Default background only (ignores OSC override)  |
 * | `VOID_TERMINAL_DATA_COLOR_CURSOR_DEFAULT`      | `VoidColorRgb*`      | Default cursor only (ignores OSC override)      |
 * | `VOID_TERMINAL_DATA_COLOR_PALETTE_DEFAULT`     | `VoidColorRgb[256]*` | Default palette only (ignores OSC overrides)    |
 *
 * For foreground, background, and cursor colors, the getters return
 * `VOID_NO_VALUE` if no color is configured (neither a default nor an
...
typedef enum VOID_ENUM_TYPED {
  ...
  VOID_SCROLL_VIEWPORT_TOP,

  /** Scroll to the bottom (active area). */
  VOID_SCROLL_VIEWPORT_BOTTOM,

  /** Scroll by a delta amount (up is negative). */
  VOID_SCROLL_VIEWPORT_DELTA,
  VOID_SCROLL_VIEWPORT_MAX_VALUE = VOID_ENUM_MAX_VALUE,
} VoidTerminalScrollViewportTag;
 ...
 */
typedef union {
  /** Scroll delta (only used with VOID_SCROLL_VIEWPORT_DELTA). Up is negative. */
  intptr_t delta;

  /** Padding for ABI compatibility. Do not use. */
  uint64_t _padding[2];
} VoidTerminalScrollViewportValue;
 ...
 */
typedef struct {
  VoidTerminalScrollViewportTag tag;
  VoidTerminalScrollViewportValue value;
} VoidTerminalScrollViewport;
...
typedef enum VOID_ENUM_TYPED {
  ...
  VOID_TERMINAL_SCREEN_PRIMARY = 0,

  /** The alternate screen. */
  VOID_TERMINAL_SCREEN_ALTERNATE = 1,
  VOID_TERMINAL_SCREEN_MAX_VALUE = VOID_ENUM_MAX_VALUE,
} VoidTerminalScreen;
 ...
 * @param userdata The userdata pointer set via VOID_TERMINAL_OPT_USERDATA
 *
 * @ingroup terminal
 */
typedef void (*VoidTerminalBellFn)(VoidTerminal terminal,
                                      void* userdata);
 ...
 * @ingroup terminal
 */
typedef bool (*VoidTerminalColorSchemeFn)(VoidTerminal terminal,
                                             void* userdata,
                                             VoidColorScheme* out_scheme);
 ...
 * @ingroup terminal
 */
typedef bool (*VoidTerminalDeviceAttributesFn)(VoidTerminal terminal,
                                                   void* userdata,
                                                   VoidDeviceAttributes* out_attrs);

/**
 * Callback function type for enquiry (ENQ, 0x05).
 *
 * Called when the terminal receives an ENQ character. Return the
 * response bytes as a VoidString. The memory must remain valid
 ...
 * @ingroup terminal
 */
typedef VoidString (*VoidTerminalEnquiryFn)(VoidTerminal terminal,
                                                   void* userdata);
 ...
 * @ingroup terminal
 */
typedef bool (*VoidTerminalSizeFn)(VoidTerminal terminal,
                                      void* userdata,
                                      VoidSizeReportSize* out_size);
 ...
 * @ingroup terminal
 */
typedef void (*VoidTerminalTitleChangedFn)(VoidTerminal terminal,
                                              void* userdata);
 ...
 * @ingroup terminal
 */
typedef void (*VoidTerminalWritePtyFn)(VoidTerminal terminal,
                                          void* userdata,
                                          const uint8_t* data,
                                          size_t len);
 ...
 * Return the version string (e.g. "myterm 1.0") as a VoidString.
 * The memory must remain valid until the callback returns. Return a
 * zero-length string to report the default "libvoid" version.
 *
 * @param terminal The terminal handle
 * @param userdata The userdata pointer set via VOID_TERMINAL_OPT_USERDATA
 ...
 * @ingroup terminal
 */
typedef VoidString (*VoidTerminalXtversionFn)(VoidTerminal terminal,
                                                     void* userdata);

/**
 * Terminal option identifiers.
 *
 * These values are used with void_terminal_set() to configure
...
typedef enum VOID_ENUM_TYPED {
   ...
   *
   * Input type: VoidTerminalWritePtyFn
   */
  VOID_TERMINAL_OPT_WRITE_PTY = 1,

  /**
   * Callback invoked when the terminal receives a BEL character
   * (0x07). Set to NULL to ignore bell events.
   *
   * Input type: VoidTerminalBellFn
   */
  VOID_TERMINAL_OPT_BELL = 2,

  /**
   * Callback invoked when the terminal receives an ENQ character
   * (0x05). Set to NULL to send no response.
   *
   * Input type: VoidTerminalEnquiryFn
   */
  VOID_TERMINAL_OPT_ENQUIRY = 3,

  /**
   * Callback invoked when the terminal receives an XTVERSION query
   * (CSI > q). Set to NULL to report the default "libvoid" string.
   *
   * Input type: VoidTerminalXtversionFn
   */
  VOID_TERMINAL_OPT_XTVERSION = 4,
   ...
   * change events.
   *
   * Input type: VoidTerminalTitleChangedFn
   */
  VOID_TERMINAL_OPT_TITLE_CHANGED = 5,

  /**
   * Callback invoked in response to XTWINOPS size queries
   * (CSI 14/16/18 t). Set to NULL to silently ignore size queries.
   *
   * Input type: VoidTerminalSizeFn
   */
  VOID_TERMINAL_OPT_SIZE = 6,
   ...
   *
   * Input type: VoidTerminalColorSchemeFn
   */
  VOID_TERMINAL_OPT_COLOR_SCHEME = 7,
   ...
   *
   * Input type: VoidTerminalDeviceAttributesFn
   */
  VOID_TERMINAL_OPT_DEVICE_ATTRIBUTES = 8,
   ...
   *
   * Input type: VoidString*
   */
  VOID_TERMINAL_OPT_TITLE = 9,
   ...
   *
   * Input type: VoidColorRgb*
   */
  VOID_TERMINAL_OPT_COLOR_FOREGROUND = 11,
   ...
   * Input type: VoidColorRgb*
   */
  VOID_TERMINAL_OPT_COLOR_BACKGROUND = 12,
   ...
   * Input type: VoidColorRgb*
   */
  VOID_TERMINAL_OPT_COLOR_CURSOR = 13,

  /**
   * Set the default 256-color palette.
   *
   * The value must point to an array of exactly 256 VoidColorRgb values.
   * A NULL value pointer resets to the built-in default palette.
   *
   * Input type: VoidColorRgb[256]*
   */
  VOID_TERMINAL_OPT_COLOR_PALETTE = 14,
   ...
   * Input type: uint64_t*
   */
  VOID_TERMINAL_OPT_KITTY_IMAGE_STORAGE_LIMIT = 15,
   ...
   * Input type: bool*
   */
  VOID_TERMINAL_OPT_KITTY_IMAGE_MEDIUM_FILE = 16,
   ...
   * Input type: bool*
   */
  VOID_TERMINAL_OPT_KITTY_IMAGE_MEDIUM_TEMP_FILE = 17,
   ...
   * Input type: bool*
   */
  VOID_TERMINAL_OPT_KITTY_IMAGE_MEDIUM_SHARED_MEM = 18,
   ...
   * Input type: size_t*
   */
  VOID_TERMINAL_OPT_APC_MAX_BYTES = 19,
   ...
   * Input type: size_t*
   */
  VOID_TERMINAL_OPT_APC_MAX_BYTES_KITTY = 20,
  VOID_TERMINAL_OPT_MAX_VALUE = VOID_ENUM_MAX_VALUE,
} VoidTerminalOption;
 ...
 * using `void_terminal_get`.
 *
 * @ingroup terminal
 */
typedef enum VOID_ENUM_TYPED {
  /** Invalid data type. Never results in any data extraction. */
  VOID_TERMINAL_DATA_INVALID = 0,
   ...
   * Output type: bool *
   */
  VOID_TERMINAL_DATA_CURSOR_PENDING_WRAP = 5,

  /**
   * The currently active screen.
   *
   * Output type: VoidTerminalScreen *
   */
  VOID_TERMINAL_DATA_ACTIVE_SCREEN = 6,
   ...
   * Output type: bool *
   */
  VOID_TERMINAL_DATA_CURSOR_VISIBLE = 7,

  /**
   * Current Kitty keyboard protocol flags.
   *
   * Output type: VoidKittyKeyFlags * (uint8_t *)
   */
  VOID_TERMINAL_DATA_KITTY_KEYBOARD_FLAGS = 8,
   ...
   *
   * Output type: VoidTerminalScrollbar *
   */
  VOID_TERMINAL_DATA_SCROLLBAR = 9,
   ...
   *
   * Output type: VoidStyle *
   */
  VOID_TERMINAL_DATA_CURSOR_STYLE = 10,
   ...
   * Output type: bool *
   */
  VOID_TERMINAL_DATA_MOUSE_TRACKING = 11,
   ...
   * to void_terminal_vt_write() or void_terminal_reset(). An empty
   * string (len=0) is returned when no title has been set.
   *
   * Output type: VoidString *
   */
  VOID_TERMINAL_DATA_TITLE = 12,
   ...
   * Output type: size_t *
   */
  VOID_TERMINAL_DATA_TOTAL_ROWS = 14,
   ...
   * Output type: size_t *
   */
  VOID_TERMINAL_DATA_SCROLLBACK_ROWS = 15,

  /**
   * The total width of the terminal in pixels.
   *
   * This is cols * cell_width_px as set by void_terminal_resize().
   *
   * Output type: uint32_t *
   */
  VOID_TERMINAL_DATA_WIDTH_PX = 16,

  /**
   * The total height of the terminal in pixels.
   *
   * This is rows * cell_height_px as set by void_terminal_resize().
   *
   * Output type: uint32_t *
   */
  VOID_TERMINAL_DATA_HEIGHT_PX = 17,

  /**
   * The effective foreground color (override or default).
   *
   * Returns VOID_NO_VALUE if no foreground color is set.
   *
   * Output type: VoidColorRgb *
   */
  VOID_TERMINAL_DATA_COLOR_FOREGROUND = 18,

  /**
   * The effective background color (override or default).
   *
   * Returns VOID_NO_VALUE if no background color is set.
   *
   * Output type: VoidColorRgb *
   */
  VOID_TERMINAL_DATA_COLOR_BACKGROUND = 19,

  /**
   * The effective cursor color (override or default).
   *
   * Returns VOID_NO_VALUE if no cursor color is set.
   *
   * Output type: VoidColorRgb *
   */
  VOID_TERMINAL_DATA_COLOR_CURSOR = 20,

  /**
   * The current 256-color palette.
   *
   * Output type: VoidColorRgb[256] *
   */
  VOID_TERMINAL_DATA_COLOR_PALETTE = 21,

  /**
   * The default foreground color (ignoring any OSC override).
   *
   * Returns VOID_NO_VALUE if no default foreground color is set.
   *
   * Output type: VoidColorRgb *
   */
  VOID_TERMINAL_DATA_COLOR_FOREGROUND_DEFAULT = 22,

  /**
   * The default background color (ignoring any OSC override).
   *
   * Returns VOID_NO_VALUE if no default background color is set.
   *
   * Output type: VoidColorRgb *
   */
  VOID_TERMINAL_DATA_COLOR_BACKGROUND_DEFAULT = 23,

  /**
   * The default cursor color (ignoring any OSC override).
   *
   * Returns VOID_NO_VALUE if no default cursor color is set.
   *
   * Output type: VoidColorRgb *
   */
  VOID_TERMINAL_DATA_COLOR_CURSOR_DEFAULT = 24,
   ...
   * Output type: VoidColorRgb[256] *
   */
  VOID_TERMINAL_DATA_COLOR_PALETTE_DEFAULT = 25,
   ...
   */
  VOID_TERMINAL_DATA_KITTY_IMAGE_STORAGE_LIMIT = 26,
   ...
   * Output type: bool *
   */
  VOID_TERMINAL_DATA_KITTY_IMAGE_MEDIUM_FILE = 27,
   ...
   * Output type: bool *
   */
  VOID_TERMINAL_DATA_KITTY_IMAGE_MEDIUM_TEMP_FILE = 28,
   ...
   * Output type: bool *
   */
  VOID_TERMINAL_DATA_KITTY_IMAGE_MEDIUM_SHARED_MEM = 29,
   ...
   * until the next mutating terminal call (e.g. void_terminal_vt_write()
   * or void_terminal_reset()).
   *
   * Returns VOID_NO_VALUE when Kitty graphics are disabled at build time.
   *
   * Output type: VoidKittyGraphics *
   */
  VOID_TERMINAL_DATA_KITTY_GRAPHICS = 30,
  VOID_TERMINAL_DATA_MAX_VALUE = VOID_ENUM_MAX_VALUE,
} VoidTerminalData;
 ...
 * @ingroup terminal
 */
VOID_API VoidResult void_terminal_new(const VoidAllocator* allocator,
                                   VoidTerminal* terminal,
                                   VoidTerminalOptions options);
 ...
 * @ingroup terminal
 */
VOID_API void void_terminal_free(VoidTerminal terminal);
 ...
 * @ingroup terminal
 */
VOID_API void void_terminal_reset(VoidTerminal terminal);
 ...
 *
 * @param terminal The terminal handle (NULL returns VOID_INVALID_VALUE)
 ...
 * @ingroup terminal
 */
VOID_API VoidResult void_terminal_resize(VoidTerminal terminal,
                                      uint16_t cols,
                                      uint16_t rows,
 ...
 * to the value for non-pointer types (e.g. VoidString*).
 * NULL clears the option to its default.
 *
 * Callbacks are invoked synchronously during void_terminal_vt_write().
 * Callbacks must not call void_terminal_vt_write() on the same
 ...
 * @ingroup terminal
 */
VOID_API VoidResult void_terminal_set(VoidTerminal terminal,
                                   VoidTerminalOption option,
                                   const void* value);
 ...
 * void_terminal_set() with VOID_TERMINAL_OPT_WRITE_PTY to install
 ...
 * @ingroup terminal
 */
VOID_API void void_terminal_vt_write(VoidTerminal terminal,
                                const uint8_t* data,
                                size_t len);
 ...
 * When using VOID_SCROLL_VIEWPORT_DELTA, set the delta field in
 ...
 * @ingroup terminal
 */
VOID_API void void_terminal_scroll_viewport(VoidTerminal terminal,
                                       VoidTerminalScrollViewport behavior);
 ...
 *             if it is reset
 * @return VOID_SUCCESS on success, VOID_INVALID_VALUE if the terminal
 *         is NULL or the mode does not correspond to a known mode
 *
 * @ingroup terminal
 */
VOID_API VoidResult void_terminal_mode_get(VoidTerminal terminal,
                                        VoidMode mode,
                                        bool* out_value);
 ...
 * @ingroup terminal
 */
VOID_API VoidResult void_terminal_mode_set(VoidTerminal terminal,
                                         VoidMode mode,
                                         bool value);
 ...
 * @ingroup terminal
 */
VOID_API VoidResult void_terminal_get(VoidTerminal terminal,
                                    VoidTerminalData data,
                                    void *out);

/**
 * Get multiple data fields from a terminal in a single call.
 *
 * This is an optimization over calling void_terminal_get()
 ...
 * @ingroup terminal
 */
VOID_API VoidResult void_terminal_get_multi(VoidTerminal terminal,
                                    size_t count,
                                    const VoidTerminalData* keys,
                                    void** values,
                                    size_t* out_written);
 ...
 * void_grid_ref_cell() and void_grid_ref_row() to extract the cell
 * and row.
 *
 ...
 * @ingroup terminal
 */
VOID_API VoidResult void_terminal_grid_ref(VoidTerminal terminal,
                                        VoidPoint point,
                                        VoidGridRef *out_ref);
 ...
 *
 * This is the inverse of void_terminal_grid_ref(): given a grid reference,
 ...
 * @return VOID_SUCCESS on success, VOID_INVALID_VALUE if the terminal
 *         or ref is NULL/invalid, VOID_NO_VALUE if the ref falls outside
 *         the requested coordinate system
 *
 * @ingroup terminal
 */
VOID_API VoidResult void_terminal_point_from_grid_ref(
    VoidTerminal terminal,
    const VoidGridRef *ref,
    VoidPointTag tag,
    VoidPointCoordinate *out);
