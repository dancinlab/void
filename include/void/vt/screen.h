...
#ifndef VOID_VT_SCREEN_H
#define VOID_VT_SCREEN_H
 ...
 * Terminal screen cell and row types.
 *
 * These types represent the contents of a terminal screen. A VoidCell
 * is a single grid cell and a VoidRow is a single row. Both are opaque
 * values whose fields are accessed via void_cell_get() and
 * void_row_get() respectively.
 *
 * @{
 */
 ...
 * must be queried via void_cell_get(). Obtain cell values from
 * terminal query APIs.
 *
 * @ingroup screen
 */
typedef uint64_t VoidCell;

/**
 * Opaque row value.
 *
 * Represents a single terminal row. The internal layout is opaque and
 * must be queried via void_row_get(). Obtain row values from
 * terminal query APIs.
 *
 * @ingroup screen
 */
typedef uint64_t VoidRow;
...
typedef enum VOID_ENUM_TYPED {
  ...
  VOID_CELL_CONTENT_CODEPOINT = 0,

  /** A codepoint that is part of a multi-codepoint grapheme cluster. */
  VOID_CELL_CONTENT_CODEPOINT_GRAPHEME = 1,

  /** No text; background color from palette. */
  VOID_CELL_CONTENT_BG_COLOR_PALETTE = 2,

  /** No text; background color as RGB. */
  VOID_CELL_CONTENT_BG_COLOR_RGB = 3,
  VOID_CELL_CONTENT_TAG_MAX_VALUE = VOID_ENUM_MAX_VALUE,
} VoidCellContentTag;
...
typedef enum VOID_ENUM_TYPED {
  ...
  VOID_CELL_WIDE_NARROW = 0,

  /** Wide character, cell width 2. */
  VOID_CELL_WIDE_WIDE = 1,

  /** Spacer after wide character. Do not render. */
  VOID_CELL_WIDE_SPACER_TAIL = 2,

  /** Spacer at end of soft-wrapped line for a wide character. */
  VOID_CELL_WIDE_SPACER_HEAD = 3,
  VOID_CELL_WIDE_MAX_VALUE = VOID_ENUM_MAX_VALUE,
} VoidCellWide;
...
typedef enum VOID_ENUM_TYPED {
  ...
  VOID_CELL_SEMANTIC_OUTPUT = 0,

  /** Content that is part of user input. */
  VOID_CELL_SEMANTIC_INPUT = 1,

  /** Content that is part of a shell prompt. */
  VOID_CELL_SEMANTIC_PROMPT = 2,
  VOID_CELL_SEMANTIC_MAX_VALUE = VOID_ENUM_MAX_VALUE,
} VoidCellSemanticContent;
...
typedef enum VOID_ENUM_TYPED {
   ...
   * Output type: bool *
   */
  VOID_CELL_DATA_HAS_HYPERLINK = 7,
   ...
   */
  VOID_CELL_DATA_SEMANTIC_CONTENT = 9,

  /**
   * The palette index for the cell's background color.
   * Only valid when content_tag is VOID_CELL_CONTENT_BG_COLOR_PALETTE.
   *
   * Output type: VoidColorPaletteIndex *
   */
  VOID_CELL_DATA_COLOR_PALETTE = 10,

  /**
   * The RGB value for the cell's background color.
   * Only valid when content_tag is VOID_CELL_CONTENT_BG_COLOR_RGB.
   *
   * Output type: VoidColorRgb *
   */
  VOID_CELL_DATA_COLOR_RGB = 11,
  VOID_CELL_DATA_MAX_VALUE = VOID_ENUM_MAX_VALUE,
} VoidCellData;
...
typedef enum VOID_ENUM_TYPED {
  ...
  VOID_ROW_SEMANTIC_NONE = 0,

  /** Prompt cells exist and this is a primary prompt line. */
  VOID_ROW_SEMANTIC_PROMPT = 1,

  /** Prompt cells exist and this is a continuation line. */
  VOID_ROW_SEMANTIC_PROMPT_CONTINUATION = 2,
  VOID_ROW_SEMANTIC_MAX_VALUE = VOID_ENUM_MAX_VALUE,
} VoidRowSemanticPrompt;
...
typedef enum VOID_ENUM_TYPED {
   ...
   * Output type: bool *
   */
  VOID_ROW_DATA_WRAP_CONTINUATION = 2,
   ...
   */
  VOID_ROW_DATA_SEMANTIC_PROMPT = 6,
   ...
   * Output type: bool *
   */
  VOID_ROW_DATA_KITTY_VIRTUAL_PLACEHOLDER = 7,
   ...
   * Output type: bool *
   */
  VOID_ROW_DATA_DIRTY = 8,
  VOID_ROW_DATA_MAX_VALUE = VOID_ENUM_MAX_VALUE,
} VoidRowData;
 ...
 * @ingroup screen
 */
VOID_API VoidResult void_cell_get(VoidCell cell,
                                VoidCellData data,
                                void *out);
 ...
 * @ingroup screen
 */
VOID_API VoidResult void_cell_get_multi(VoidCell cell,
                                     size_t count,
                                     const VoidCellData* keys,
                                     void** values,
                                     size_t* out_written);
 ...
 * @ingroup screen
 */
VOID_API VoidResult void_row_get(VoidRow row,
                               VoidRowData data,
                               void *out);
 ...
 * @ingroup screen
 */
VOID_API VoidResult void_row_get_multi(VoidRow row,
                                    size_t count,
                                    const VoidRowData* keys,
                                    void** values,
                                    size_t* out_written);
