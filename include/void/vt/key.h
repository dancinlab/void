...
#ifndef VOID_VT_KEY_H
#define VOID_VT_KEY_H
 ...
 * ## Basic Usage
 *
 * 1. Create an encoder instance with void_key_encoder_new()
 * 2. Configure encoder options with void_key_encoder_setopt()
 *    or void_key_encoder_setopt_from_terminal() if you have a 
 *    VoidTerminal.
 * 3. For each key event:
 *    - Create a key event with void_key_event_new()
 *    - Set event properties (action, key, modifiers, etc.)
 *    - Encode with void_key_encoder_encode()
 *    - Free the event with void_key_event_free()
 *    - Note: You can also reuse the same key event multiple times by
 *      changing its properties.
 * 4. Free the encoder with void_key_encoder_free() when done
 *
 ...
 *
 * When you have a VoidTerminal, you can sync its modes (cursor key
 ...
 * void_key_encoder_new(NULL, &encoder);
 * void_key_encoder_setopt_from_terminal(encoder, terminal);
 *
 * // Encode a key event using the terminal-derived options
 * char buf[128];
 * size_t written = 0;
 * void_key_encoder_encode(encoder, event, buf, sizeof(buf), &written);
 *
