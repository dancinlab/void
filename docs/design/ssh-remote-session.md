...
- L0 보호: `src/sys/pty.hexa`, `src/sys/term.hexa`, 기타 L0 파일은 수정 금지. 신규 API는 L1/Layer 4(ui)에만 추가.
...
`src/net/ssh/` 아래에 transport(chacha20-poly1305/ed25519), userauth, channel 레이어를 모두 `.hexa`로 작성.
...
### 5.1 `src/ui/tab_mux.hexa` (수정 — 공개 API 추가)
...
### 5.3 `src/ui/tab_session.hexa` (수정)
...
| 영역 | 변경 | 규모 |
|---|---|---|
| `src/sys/pty.hexa` | `pty_spawn_cmd(master, slave, argv)` 추가 (L0 확장, 승인 필요) | ~15 LOC |
| `src/ui/tab_mux.hexa` | `mux_spawn_remote` 추가 | ~20 LOC |
| `src/ui/tab_input.hexa` | Alt+s 바인딩 + 미니 프롬프트 | ~40 LOC |
| `src/ui/tab_session.hexa` | `kind`/`remote_host` 직렬화 | ~10 LOC |
