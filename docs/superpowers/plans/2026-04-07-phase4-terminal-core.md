...
**Files:**
- Modify: `src/terminal/vt_parser.hexa:61-137` (apply_sgr)
- Modify: `src/render/ansi.hexa:36-62` (ansi_fg, ansi_bg)

**RGB encoding:** `color = 256 + R*65536 + G*256 + B` (fits in i64)

- [ ] **Step 1: Parser — SGR 38;2;R;G;B (TrueColor FG)**

In `src/terminal/vt_parser.hexa`, inside `apply_sgr()`, modify the `p == 38` branch (line ~106):
...
In `src/render/ansi.hexa`, modify `ansi_fg()` (line ~36):
...
Run: `hexa run src/main.hexa` 후 셸에서:
```bash
...
```bash
git add src/terminal/vt_parser.hexa src/render/ansi.hexa
...
**Files:**
- Modify: `src/sys/term.hexa` (ioctl 기반 사이즈 감지 추가)
- Modify: `src/terminal/grid.hexa` (그리드 리사이즈 함수)
- Modify: `src/main.hexa` (리사이즈 폴링 루프)

- [ ] **Step 1: term.hexa — ioctl 기반 터미널 사이즈 (빠른 버전)**

`src/sys/term.hexa`에 ioctl 기반 사이즈 함수 추가 (기존 term_size는 유지, 새 함수 추가):
...
`src/main.hexa`에서 메인 루프 수정. `let frame_count = 0` 아래에 추가:
...
Run: `hexa run src/main.hexa` 후 터미널 창 크기를 마우스로 조절
Expected: 그리드가 새 크기에 맞춰 재렌더링, 기존 내용 유지, statusbar에 새 크기 표시

- [ ] **Step 5: Commit**

```bash
git add src/sys/term.hexa src/terminal/grid.hexa src/main.hexa
...
**Files:**
- Modify: `src/terminal/grid.hexa` (alt grid 저장/복원)
- Modify: `src/terminal/vt_parser.hexa:221-241` (CSI ?1049h/l 디스패치)
...
```bash
git add src/terminal/grid.hexa src/terminal/vt_parser.hexa
...
**Files:**
- Modify: `src/terminal/vt_parser.hexa` (마우스 모드 CSI 추가)
- Modify: `src/main.hexa` (마우스 입력 파싱 + PTY 전달)
...
`src/main.hexa`에 마우스 입력 인코딩 함수 추가 (메인 루프 전):
...
```bash
git add src/terminal/vt_parser.hexa src/main.hexa
...
**Files:**
- Modify: `src/main.hexa` (키 입력 가로채기 + 스크롤 오프셋)
- Modify: `src/render/ansi.hexa` (스크롤 오프셋 반영 렌더링)
...
`src/render/ansi.hexa`에 스크롤 오프셋 렌더 함수 추가:
...
```bash
git add src/main.hexa src/render/ansi.hexa
...
**Files:**
- Modify: `src/main.hexa` (prev_grid 추적 + render_grid 사용)
...
**Files:**
- Modify: `src/terminal/grid.hexa` (새 플래그 상수)
- Modify: `src/terminal/vt_parser.hexa` (SGR 확장 + charset 전환)
- Modify: `src/render/ansi.hexa` (새 속성 출력)
...
```bash
git add src/terminal/grid.hexa src/terminal/vt_parser.hexa src/render/ansi.hexa
...
**Files:**
- Modify: `src/terminal/vt_parser.hexa` (dispatch_csi 확장)
- Modify: `src/terminal/grid.hexa` (필요한 그리드 연산)

- [ ] **Step 1: CSI P — Delete Characters (DCH)**

`src/terminal/grid.hexa`에 추가:
...
`src/terminal/vt_parser.hexa`의 `dispatch_csi()`에 추가 (line ~206 부근, `M` 분기 뒤):
...
`src/main.hexa`에서 vt_process 결과 처리 부분에 reply 전달:
...
```bash
git add src/terminal/vt_parser.hexa src/terminal/grid.hexa src/main.hexa
...
**Files:**
- Modify: `src/terminal/vt_parser.hexa` (OSC 777 디스패치)
- Modify: `src/terminal/protocol.hexa` (프로토콜 핸들러)
- Modify: `src/main.hexa` (use protocol, 프로토콜 결과 처리)

- [ ] **Step 1: protocol.hexa — 프로토콜 파싱 함수**

`src/terminal/protocol.hexa` 재작성:
...
```bash
git add src/terminal/protocol.hexa src/terminal/vt_parser.hexa src/main.hexa
