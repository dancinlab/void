...
class TerminalController: BaseTerminalController, TabGroupCloseCoordinator.Controller {
    override var windowNibName: NSNib.Name? {
        ...
        let config = appDelegate.void.config
    ...
    init(_ void: VD.App,
        ...
        super.init(void, baseConfig: base, surfaceTree: tree)
    ...
    static func newWindow(
        ...
        } else if let fullscreenMode = void.config.windowFullscreen {
    ...
    static func newWindow(
        ...
        let c = TerminalController.init(void, withSurfaceTree: tree)
        ...
        if let undoManager = c.undoManager {
            ...
            undoManager.registerUndo(
                ...
                undoManager.registerUndo(
                    ...
                    _ = TerminalController.newWindow(void, tree: tree)
                }
            }
        }
    ...
    static func newTab(
        ...
        let controller = TerminalController.init(void, withBaseConfig: baseConfig)
        ...
        if window.tabbingMode != .disallowed {
            ...
            switch void.config.windowNewTabPosition {
            case "end":
    ...
    @objc private func voidConfigDidChange(_ notification: Notification) {
        ...
        /// ``Void/Void/SurfaceView/derivedConfig`` then
    ...
    func relabelTabs() {
        ...
        if let windows = window?.tabbedWindows as? [TerminalWindow] {
            for (tab, window) in zip(1..., windows) {
                ...
                if let equiv = void.config.keyboardShortcut(for: "goto_tab:\(tab)") {
    ...
    private func syncAppearance(_ surfaceConfig: VD.SurfaceView.DerivedConfig) {
        ...
        terminalViewContainer?.voidConfigDidChange(void.config, preferredBackgroundColor: window.preferredBackgroundColor)
    }
    ...
    func closeTabImmediately(registerRedo: Bool = true) {
        ...
        if let undoManager, let undoState {
            ...
            undoManager.registerUndo(
                ...
                let newController = TerminalController(void, with: undoState)
    ...
    private func registerUndoForCloseWindow() {
        ...
        guard let tabGroup = window.tabGroup,
            ...
            if let undoState {
                ...
                undoManager.registerUndo(
                    withTarget: void,
                    expiresAfter: undoExpiration) { void in
        ...
        undoManager.registerUndo(
            ...
            let controllers = undoStates.map { undoState in
                TerminalController(void, with: undoState)
            }
    ...
    override func windowDidLoad() {
        ...
        let container = TerminalViewContainer {
            TerminalView(void: void, viewModel: self, delegate: self)
        }
        ...
        // In various situations, macOS automatically tabs new windows. Void handles
        ...
        if !window.styleMask.contains(.fullScreen) {
            ...
            // Since Void manages tabbing manually this will never be more than one
    ...
    @IBAction func newWindow(_ sender: Any?) {
        ...
        void.newWindow(surface: surface)
    }
    ...
    @IBAction func toggleVoidFullScreen(_ sender: Any?) {
        guard let surface = focusedSurface?.surface else { return }
        void.toggleFullscreen(surface: surface)
    }

    @IBAction func toggleTerminalInspector(_ sender: Any?) {
        guard let surface = focusedSurface?.surface else { return }
        void.toggleTerminalInspector(surface: surface)
    }
    ...
    @objc private func onGotoTab(notification: SwiftUI.Notification) {
        ...
        if tabIndex <= 0 {
            ...
            if tabIndex == VOID_GOTO_TAB_PREVIOUS.rawValue {
                if selectedIndex == 0 {
                    finalIndex = tabbedWindows.count - 1
                } else {
                    finalIndex = selectedIndex - 1
                }
            } else if tabIndex == VOID_GOTO_TAB_NEXT.rawValue {
                if selectedIndex == tabbedWindows.count - 1 {
                    finalIndex = 0
                } else {
                    finalIndex = selectedIndex + 1
                }
            } else if tabIndex == VOID_GOTO_TAB_LAST.rawValue {
                finalIndex = tabbedWindows.count - 1
            } else {
                return
            }
