...
class BaseTerminalController: NSWindowController,
    ...
    private var appliedColorScheme: void_color_scheme_e?
    ...
    /// Stable orientation hint for grid rebuilds. A portrait screen should keep
    /// portrait-biased grids even when a transient window frame reports wider
    /// than tall during split/tree updates.
    private var splitGridPrefersTall: Bool {
    ...
    init(_ void: VD.App,
        ...
        center.addObserver(
            self,
            selector: #selector(voidConfigDidChangeBase(_:)),
            name: .voidConfigDidChange,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(voidCommandPaletteDidToggle(_:)),
            name: .voidCommandPaletteDidToggle,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(voidMaximizeDidToggle(_:)),
        ...
        center.addObserver(
            self,
            selector: #selector(voidDidChangePin(_:)),
        ...
        center.addObserver(
            self,
            selector: #selector(voidSurfaceDragEndedNoTarget(_:)),
            name: .voidSurfaceDragEndedNoTarget,
            object: nil)
    ...
    func surfaceTreeDidChange(from: SplitTree<VD.SurfaceView>, to: SplitTree<VD.SurfaceView>) {
        ...
        // vsync stopped firing when that window closed. viewDidMoveToWindow
        // would handle it, but SwiftUI's NSViewRepresentable wrapper can
        // mount the surface into its new scroll view *before* the scroll
        // view joins the new window hierarchy — in that order, the hook
        // sees window==nil and early-returns. Defer to the next runloop
        // so SwiftUI has fully attached the view tree, then refresh.
        let before = Set(from.map { ObjectIdentifier($0) })
        let newlyAdded = to.filter { !before.contains(ObjectIdentifier($0)) }
        if !newlyAdded.isEmpty {
            DispatchQueue.main.async {
                for surface in newlyAdded { surface.refreshRendererPipeline() }
            }
        }
    }

    /// Update all surfaces with the focus state. This ensures that libvoid has an accurate view about
    ...
    func closeSurface(
        ...
        // due to SwiftUI bugs (see Void #560). To repeat from #560, the bug is that
    ...
    private func removeSurfaceNode(_ node: SplitTree<VD.SurfaceView>.Node) {
        ...
        // Clear pin/edge-snap state here because a close-triggered grid
        // reflow should normalize the grid, not preserve an old carved-out
        // subtree shape.
        let tabGroupWins = self.window?.tabGroup?.windows.count ?? 0
        if tabGroupWins <= 1, next.isSplit {
            let leaves = Array(next)
            if leaves.count >= 2 {
                let prefersTall = splitGridPrefersTall
                clearGridPinState(in: leaves)
                next = SplitTree<VD.SurfaceView>.grid(
                    views: leaves,
    ...
    @objc private func voidConfigDidChangeBase(_ notification: Notification) {
    ...
    @objc private func voidDidNewSplit(_ notification: Notification) {
        ...
        case VOID_SPLIT_DIRECTION_DOWN: splitDirection = .down
        case VOID_SPLIT_DIRECTION_UP: splitDirection = .up
        default: return
        }
    ...
    @objc private func voidDidFocusGridCell(_ notification: Notification) {
        ...
        // Single VD.moveFocus call — it internally dispatches to main and
    ...
    @objc private func voidDidAddGridCell(_ notification: Notification) {
        ...
        // Rebuild the entire tree as a fresh grid. Insert the new cell
        // IMMEDIATELY AFTER the focused cell rather than appending so the new
        // pane lands adjacent to where the user was working, not at the far
        // end of the grid.
        var ordered = leaves
    ...
    private func clearGridPinState(in views: [VD.SurfaceView]) {
        for view in views {
            view.isPinned = false
    ...
    private func flattenTabsToGrid(
        ...
        let prefersTall = target.splitGridPrefersTall
    ...
    @objc private func voidSurfaceDragEndedNoTarget(_ notification: Notification) {
        ...
        _ = TerminalController.newWindow(
            void,
            tree: newTree,
            position: notification.userInfo?[Notification.Name.voidSurfaceDragEndedNoTargetPointKey] as? NSPoint,
            confirmUndo: false)
    }
    ...
    private func computeTitle(title: String, bell: Bool) -> String {
        ...
        if bell && void.config.bellFeatures.contains(.title) {
            // Sentinel prefix; TerminalWindow.attributedTitle swaps it for
            // a brand-green image attachment so the rendered indicator
            // matches the per-cell SwiftUI Circle in grid mode.
            result = "\(LiveIndicator.titlePrefix)\(result)"
        }
    ...
    /// Promote `source` to occupy one row's height (top/bottom) or one
    /// column's width (left/right) of the natural grid for the current pane
    /// count, leaving the rest as a clean balanced grid in the other side.
    /// Examples:
    ///   - 4 panes (2×2) snapped right → source = 1/2 width (1 col)
    ///   - 6 panes (3×2) snapped right → source = 1/3 width (1 col of 3)
    ///   - 6 panes (3×2) snapped top   → source = 1/2 height (1 row of 2)
    /// Already-pinned panes other than the source keep their pin state and
    /// their slots in the rebuilt remainder. No-op if source is alone.
    private func splitDidEdgeSnap(
        ...
        guard let newRoot = applyEdgeSnap(
            ...
            zone: zone,
            in: surfaceTree,
            prefersTall: splitGridPrefersTall) else { return }
    ...
    private func applyEdgeSnap(
        ...
        switch zone {
        case .top, .bottom: sourceFraction = 1.0 / Double(dims.rows)
        case .left, .right: sourceFraction = 1.0 / Double(dims.cols)
        }

        let direction: SplitTree<VD.SurfaceView>.Direction
        let sourceOnLeft: Bool
        switch zone {
        case .top:    direction = .vertical;   sourceOnLeft = true
        case .bottom: direction = .vertical;   sourceOnLeft = false
    ...
    /// panes' positions found in `existing`. Pure-unpinned subtrees of
    ...
    private func rebuildAroundPins(
        views: [VD.SurfaceView],
        existing: SplitTree<VD.SurfaceView>.Node?,
        prefersTall: Bool
    ) -> SplitTree<VD.SurfaceView>.Node? {
        guard !views.isEmpty else { return nil }
        // Fast path: no pinned panes anywhere among `views` → just regrid.
        if views.allSatisfy({ !$0.isPinned }) {
            return SplitTree<VD.SurfaceView>.grid(
                views: views, prefersTall: prefersTall).root
        }
        // Walk `existing` and replicate its structure for pinned subtrees.
        // Unpinned regions get rebuilt as grids over their leaves intersected
        // with `views`.
    ...
    /// Pin-state toggled — refresh the tree so unpinned regions reflow
    /// (in particular: a freshly-unpinned pane rejoins its enclosing
    /// pure-unpinned region's grid). No-op if the toggle came from a view
    /// outside this controller's tree.
    @objc private func voidDidChangePin(_ notification: Notification) {
        guard let view = notification.object as? VD.SurfaceView,
              surfaceTree.contains(view) else { return }
        guard let root = surfaceTree.root else { return }
        ...
        let newTree = SplitTree<VD.SurfaceView>(root: newRoot, zoomed: surfaceTree.zoomed)
        // Pin toggle isn't undoable on its own — the tree shape change is
        // a side effect of a UI flip, not a discrete user action worth a
        // separate undo step.
        replaceSurfaceTree(
            newTree,
            moveFocusTo: view,
    ...
    func performAction(_ action: String, on surfaceView: VD.SurfaceView) {
        ...
        _ = action.withCString { cString in
            void_surface_binding_action(surface, cString, UInt(len - 1))
        }
    }
    ...
    func toggleBackgroundOpacity() {
        ...
        guard void.config.backgroundOpacity < 1 else { return }
    ...
    func windowDidChangeOcclusionState(_ notification: Notification) {
        ...
        for view in surfaceTree {
            if let surface = view.surface {
                void_surface_set_occlusion(surface, visible)
            }
        }
    }
    ...
    @IBAction func close(_ sender: Any) {
        ...
        void.requestClose(surface: surface)
    }
    ...
    @IBAction func splitRight(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        void.split(surface: surface, direction: VOID_SPLIT_DIRECTION_RIGHT)
    }

    @IBAction func splitLeft(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        void.split(surface: surface, direction: VOID_SPLIT_DIRECTION_LEFT)
    }

    @IBAction func splitDown(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        void.split(surface: surface, direction: VOID_SPLIT_DIRECTION_DOWN)
    }

    @IBAction func splitUp(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        void.split(surface: surface, direction: VOID_SPLIT_DIRECTION_UP)
    }

    @IBAction func splitZoom(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        void.splitToggleZoom(surface: surface)
    }
    ...
    @IBAction func moveSplitDividerUp(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        void.splitResize(surface: surface, direction: .up, amount: 10)
    }

    @IBAction func moveSplitDividerDown(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        void.splitResize(surface: surface, direction: .down, amount: 10)
    }

    @IBAction func moveSplitDividerLeft(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        void.splitResize(surface: surface, direction: .left, amount: 10)
    }
    ...
    @IBAction func increaseFontSize(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        void.changeFontSize(surface: surface, .increase(1))
    }

    @IBAction func decreaseFontSize(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        void.changeFontSize(surface: surface, .decrease(1))
    }

    @IBAction func resetFontSize(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        void.changeFontSize(surface: surface, .reset)
    }
    ...
    @objc func resetTerminal(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        void.resetTerminal(surface: surface)
    }
...
extension BaseTerminalController: NSMenuItemValidation {
    ...
    ///
    /// Calling ``void_surface_set_color_scheme`` triggers
    ...
    func updateColorSchemeForSurfaceTree() {
        ...
        for surfaceView in surfaceTree {
            if let surface = surfaceView.surface {
                void_surface_set_color_scheme(surface, scheme)
            }
        }
...
extension Notification.Name {
    ...
    static let terminalWindowBellDidChangeNotification = Notification.Name("com.dancinlab.void.terminalWindowBellDidChange")
