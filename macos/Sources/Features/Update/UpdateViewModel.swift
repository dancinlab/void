...
enum UpdateState: Equatable {
    ...
    struct UpdateAvailable {
        ...
        var releaseNotes: ReleaseNotes? {
            let currentCommit = Bundle.main.infoDictionary?["VoidCommit"] as? String
    ...
    enum ReleaseNotes {
        ...
        init?(displayVersionString: String, currentCommit: String?) {
            ...
            if let semver = Self.extractSemanticVersion(from: version) {
                ...
                if let url = URL(string: "https://void.org/docs/install/release-notes/\(slug)") {
                    self = .tagged(url)
                    return
                }
            }
    ...
    struct Installing {
        /// True if this state is triggered by ``Void/UpdateDriver/updater(_:willInstallUpdateOnQuit:immediateInstallationBlock:)``
