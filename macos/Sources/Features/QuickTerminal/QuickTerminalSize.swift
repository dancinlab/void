...
/// Represents the Void `quick-terminal-size` configuration. See the documentation for
...
struct QuickTerminalSize {
    ...
    init(from cStruct: void_config_quick_terminal_size_s) {
    ...
    enum Size {
        ...
        init?(from cStruct: void_quick_terminal_size_s) {
            switch cStruct.tag {
            case VOID_QUICK_TERMINAL_SIZE_NONE:
                return nil
            case VOID_QUICK_TERMINAL_SIZE_PERCENTAGE:
                self = .percentage(cStruct.value.percentage)
            case VOID_QUICK_TERMINAL_SIZE_PIXELS:
