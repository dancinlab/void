...
extension VD {
    ...
    struct InspectableSurface: View {
        ...
        var body: some View {
            ...
            ZStack {
                if !surfaceView.inspectorVisible {
                    ...
                    SplitView(.vertical, $split, dividerColor: void.config.splitDividerColor, left: {
                        SurfaceWrapper(surfaceView: surfaceView, isSplit: isSplit)
                    }, right: {
        ...
        private func onControlInspector(_ notification: SwiftUI.Notification) {
            ...
            guard let mode = modeAny as? void_action_inspector_e else { return }

            switch mode {
            case VOID_INSPECTOR_TOGGLE:
                surfaceView.inspectorVisible = !surfaceView.inspectorVisible

            case VOID_INSPECTOR_SHOW:
                surfaceView.inspectorVisible = true

            case VOID_INSPECTOR_HIDE:
    ...
    class InspectorView: MTKView, NSTextInputClient {
        ...
        override func scrollWheel(with event: NSEvent) {
            ...
            var momentum: void_input_mouse_momentum_e = VOID_MOUSE_MOMENTUM_NONE
            switch event.momentumPhase {
            case .began:
                momentum = VOID_MOUSE_MOMENTUM_BEGAN
            case .stationary:
                momentum = VOID_MOUSE_MOMENTUM_STATIONARY
            case .changed:
                momentum = VOID_MOUSE_MOMENTUM_CHANGED
            case .ended:
                momentum = VOID_MOUSE_MOMENTUM_ENDED
            case .cancelled:
                momentum = VOID_MOUSE_MOMENTUM_CANCELLED
            case .mayBegin:
                momentum = VOID_MOUSE_MOMENTUM_MAY_BEGIN
            default:
                break
            }
        ...
        override func keyUp(with event: NSEvent) {
            keyAction(VOID_ACTION_RELEASE, event: event)
        }
        ...
        override func flagsChanged(with event: NSEvent) {
            ...
            if mods.rawValue & mod != 0 { action = VOID_ACTION_PRESS }
