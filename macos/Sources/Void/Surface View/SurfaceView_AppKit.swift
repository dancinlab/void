...
extension VD {
    ...
    class SurfaceView: OSSurfaceView, Codable, Identifiable {
                ...
                if let searchState {
                    ...
                    searchNeedleCancellable = searchState.$needle
                        ...
                        .sink { [weak self] needle in
                            ...
                            void_surface_binding_action(surface, action, UInt(action.lengthOfBytes(using: .utf8)))
                        }
        ...
        /// The configuration derived from the Void config so we don't need to rely on references.
        ...
        // detected with the set_password_input_cb on the Void state.
        ...
        init(_ app: void_app_t, baseConfig: SurfaceConfiguration? = nil, uuid: UUID? = nil) {
            self.markedText = NSMutableAttributedString()

            // Our initial config always is our application wide config.
            if let appDelegate = NSApplication.shared.delegate as? AppDelegate {
                self.derivedConfig = DerivedConfig(appDelegate.void.config)
            } else {
            ...
            cachedScreenContents = .init(duration: .milliseconds(500)) { [weak self] in
                ...
                guard let surface = self.surface else { return "" }
                var text = void_text_s()
                let sel = void_selection_s(
                    top_left: void_point_s(
                        tag: VOID_POINT_SCREEN,
                        coord: VOID_POINT_COORD_TOP_LEFT,
                        x: 0,
                        y: 0),
                    bottom_right: void_point_s(
                        tag: VOID_POINT_SCREEN,
                        coord: VOID_POINT_COORD_BOTTOM_RIGHT,
                        x: 0,
                        y: 0),
                    rectangle: false)
                guard void_surface_read_text(surface, sel, &text) else { return "" }
                defer { void_surface_free_text(surface, &text) }
            ...
            center.addObserver(
                self,
                selector: #selector(voidColorDidChange(_:)),
        ...
        override func focusDidChange(_ focused: Bool) {
            ...
            // Notify libvoid
            void_surface_set_focus(surface, focused)
        ...
        override func sizeDidChange(_ size: CGSize) {
            // Void wants to know the actual framebuffer size... It is very important
        ...
        private func setSurfaceSize(width: UInt32, height: UInt32) {
            ...
            void_surface_set_size(surface, width, height)

            // Update our cached size metrics
            let size = void_surface_size(surface)
        ...
        func setCursorShape(_ shape: void_action_mouse_shape_e) {
            switch shape {
            case VOID_MOUSE_SHAPE_DEFAULT:
                pointerStyle = .default

            case VOID_MOUSE_SHAPE_TEXT:
                pointerStyle = .horizontalText

            case VOID_MOUSE_SHAPE_GRAB:
                pointerStyle = .grabIdle

            case VOID_MOUSE_SHAPE_GRABBING:
                pointerStyle = .grabActive

            case VOID_MOUSE_SHAPE_POINTER:
                pointerStyle = .link

            case VOID_MOUSE_SHAPE_W_RESIZE:
                pointerStyle = .resizeLeft

            case VOID_MOUSE_SHAPE_E_RESIZE:
                pointerStyle = .resizeRight

            case VOID_MOUSE_SHAPE_N_RESIZE:
                pointerStyle = .resizeUp

            case VOID_MOUSE_SHAPE_S_RESIZE:
                pointerStyle = .resizeDown

            case VOID_MOUSE_SHAPE_NS_RESIZE:
                pointerStyle = .resizeUpDown

            case VOID_MOUSE_SHAPE_EW_RESIZE:
                pointerStyle = .resizeLeftRight

            case VOID_MOUSE_SHAPE_VERTICAL_TEXT:
                pointerStyle = .verticalText

            case VOID_MOUSE_SHAPE_CONTEXT_MENU:
                pointerStyle = .contextMenu

            case VOID_MOUSE_SHAPE_CROSSHAIR:
                pointerStyle = .crosshair

            case VOID_MOUSE_SHAPE_NOT_ALLOWED:
        ...
        @objc private func onUpdateRendererHealth(notification: SwiftUI.Notification) {
            ...
            guard let health = healthAny as? void_action_renderer_health_e else { return }
            DispatchQueue.main.async { [weak self] in
                self?.healthy = health == VOID_RENDERER_HEALTH_HEALTHY
            }
        }
        ...
        @objc private func voidDidEndKeySequence(notification: SwiftUI.Notification) {
            DispatchQueue.main.async { [weak self] in
                self?.keySequence = []
            }
        }
        ...
        @objc private func voidConfigDidChange(_ notification: SwiftUI.Notification) {
        ...
        @objc private func voidBellDidRing(_ notification: SwiftUI.Notification) {
        ...
        @objc private func windowDidChangeScreen(notification: SwiftUI.Notification) {
            ...
            // When the window changes screens, we need to update libvoid with the screen
            // ID. If vsync is enabled, this will be used with the CVDisplayLink to ensure
            // the proper refresh rate is going.
            void_surface_set_display_id(surface, screen.displayID ?? 0)
        ...
        func refreshRendererPipeline(attempt: Int = 0) {
            ...
            guard let surface = self.surface else { return }
            // Before flatten, sibling tabs' windowDidChangeOcclusionState
            // fires with visible=false as their windows close. That sets
            // void_surface_set_occlusion(surface, false) and libvoid stops
            // rendering the surface. The surface then migrates to the host
            // grid window but the occlusion flag is stuck at false → the
            // cell receives keys but never repaints.
            void_surface_set_occlusion(surface, true)
            void_surface_set_display_id(surface, self.window?.screen?.displayID ?? 0)
            viewDidChangeBackingProperties()
            // sizeDidChange is what actually forwards to void_surface_set_size —
            // calling setFrameSize alone just adjusts NSView.frame without
            // informing libvoid. Bump-and-restore both sides to force a
            // re-submit even if the size already matches libvoid's cache.
            let current = frame.size
            if current.width > 1 && current.height > 1 {
                sizeDidChange(CGSize(width: current.width + 1, height: current.height))
                sizeDidChange(current)
            }
        ...
        override func viewDidMoveToWindow() {
            ...
            // flattenTabsToGrid migrates surfaces from sibling tabs whose
            // windows get closed mid-flow; their occlusion flag stays
            // false in libvoid and no draw is ever submitted from the
            // new host grid. refreshRendererPipeline un-sticks that +
            // rebinds CVDisplayLink / backing / size.
            refreshRendererPipeline()
        }
        ...
        override func viewDidChangeBackingProperties() {
            ...
            void_surface_set_content_scale(surface, xScale, yScale)
        ...
        override func mouseUp(with event: NSEvent) {
            ...
            // Release pressure
            void_surface_mouse_pressure(surface, 0, 0)
        }
        ...
        override func pressureChange(with event: NSEvent) {
            ...
            // Notify Void first. We do this because this will let Void handle
            // state setup that we'll need for later pressure handling (such as
            // QuickLook)
            void_surface_mouse_pressure(surface, UInt32(event.stage), Double(event.pressure))
            ...
            guard UserDefaults.void.bool(forKey: "com.apple.trackpad.forceClick") else { return }
        ...
        override func keyDown(with event: NSEvent) {
            ...
            // so we can't use translationModsVoid directly. Instead, we just check
            // for exact states and set them.
            var translationMods = event.modifierFlags
            for flag in [NSEvent.ModifierFlags.shift, .control, .option, .command] {
                if translationModsVoid.contains(flag) {
            ...
            let action = event.isARepeat ? VOID_ACTION_REPEAT : VOID_ACTION_PRESS
            ...
            if let list = keyTextAccumulator, list.count > 0 {
                ...
                _ = keyAction(
                    ...
                    event: event,
                    translationEvent: translationEvent,
                    text: translationEvent.voidCharacters,
        ...
        override func keyUp(with event: NSEvent) {
            _ = keyAction(VOID_ACTION_RELEASE, event: event)
        }
        ...
        override func performKeyEquivalent(with event: NSEvent) -> Bool {
            ...
            let bindingFlags = surfaceModel.flatMap { surface in
                ...
                return (event.characters ?? "").withCString { ptr in
                    voidEvent.text = ptr
                    return surface.keyIsBinding(voidEvent)
                }
            }
                ...
                if keySequence.isEmpty,
                   ...
                   bindingFlags.contains(.consumed) {
                    if let appDelegate = NSApp.delegate as? AppDelegate,
                       appDelegate.performVoidBindingMenuKeyEquivalent(with: event) {
                        return true
                    }
                }
        ...
        override func flagsChanged(with event: NSEvent) {
            ...
            case 0x39: mod = VOID_MODS_CAPS.rawValue
            case 0x38, 0x3C: mod = VOID_MODS_SHIFT.rawValue
            case 0x3B, 0x3E: mod = VOID_MODS_CTRL.rawValue
            case 0x3A, 0x3D: mod = VOID_MODS_ALT.rawValue
            case 0x37, 0x36: mod = VOID_MODS_SUPER.rawValue
            default: return
            }
        ...
        private func keyAction(
            ...
            var key_ev = event.voidKeyEvent(action, translationMods: translationEvent?.modifierFlags)
            key_ev.composing = composing

            // For text, we only encode UTF8 if we don't have a single control
            // character. Control characters are encoded by Void itself.
            // Without this, `ctrl+enter` does the wrong thing.
            if let text, text.count > 0,
               let codepoint = text.utf8.first, codepoint >= 0x20 {
                return text.withCString { ptr in
                    key_ev.text = ptr
                    return void_surface_key(surface, key_ev)
                }
            } else {
                return void_surface_key(surface, key_ev)
            }
        ...
        override func quickLook(with event: NSEvent) {
            ...
            var text = void_text_s()
            guard void_surface_quicklook_word(surface, &text) else { return super.quickLook(with: event) }
            defer { void_surface_free_text(surface, &text) }
            ...
            if let fontRaw = void_surface_quicklook_font(surface) {
                // Memory management here is wonky: void_surface_quicklook_font
            ...
            // Void coordinate system is top-left, convert to bottom-left for AppKit
        ...
        @IBAction func splitRight(_ sender: Any) {
            guard let surface = self.surface else { return }
            void_surface_split(surface, VOID_SPLIT_DIRECTION_RIGHT)
        }

        @IBAction func splitLeft(_ sender: Any) {
            guard let surface = self.surface else { return }
            void_surface_split(surface, VOID_SPLIT_DIRECTION_LEFT)
        }

        @IBAction func splitDown(_ sender: Any) {
            guard let surface = self.surface else { return }
            void_surface_split(surface, VOID_SPLIT_DIRECTION_DOWN)
        }

        @IBAction func splitUp(_ sender: Any) {
            guard let surface = self.surface else { return }
            void_surface_split(surface, VOID_SPLIT_DIRECTION_UP)
        }
        ...
        struct DerivedConfig {
            ...
            init(_ config: VD.Config) {
                ...
                self.windowAppearance = .init(voidConfig: config)
...
extension VD.SurfaceView: NSTextInputClient {
    ...
    func selectedRange() -> NSRange {
        ...
        // Get our range from the Void API. There is a race condition between getting the
        ...
        var text = void_text_s()
        guard void_surface_read_selection(surface, &text) else { return NSRange() }
        defer { void_surface_free_text(surface, &text) }
    ...
    func attributedSubstring(forProposedRange range: NSRange, actualRange: NSRangePointer?) -> NSAttributedString? {
        ...
        var text = void_text_s()
        guard void_surface_read_selection(surface, &text) else { return nil }
        defer { void_surface_free_text(surface, &text) }
    ...
    func firstRect(forCharacterRange range: NSRange, actualRange: NSRangePointer?) -> NSRect {
        ...
        // Void will tell us where it thinks an IME keyboard should render.
        var x: Double = 0
        var y: Double = 0
        ...
        if range.length > 0 && range != self.selectedRange() {
            // QuickLook
            var text = void_text_s()
            if void_surface_read_selection(surface, &text) {
                ...
                // Free our text
                void_surface_free_text(surface, &text)
            } else {
                void_surface_ime_point(surface, &x, &y, &width, &height)
            }
        } else {
            void_surface_ime_point(surface, &x, &y, &width, &height)
        }
        ...
        // Void coordinates are in top-left (0, 0) so we have to convert to
    ...
    /// Sync the preedit state based on the markedText value to libvoid
...
extension VD.SurfaceView: NSServicesMenuRequestor {
    override func validRequestor(
        ...
        if (returnType == nil || receivable.contains(returnType!)) &&
            ...
            // performance hits because `void_surface_has_selection` isn't free.
            if let sendType, sendableRequiresSelection.contains(sendType) {
                if surface == nil || !void_surface_has_selection(surface) {
    ...
    func writeSelection(
        ...
        // Read the selection
        var text = void_text_s()
        guard void_surface_read_selection(surface, &text) else { return false }
        defer { void_surface_free_text(surface, &text) }
    ...
    func readSelection(from pboard: NSPasteboard) -> Bool {
        ...
        str.withCString { ptr in
            ...
            void_surface_text(surface, ptr, UInt(len - 1))
        }
...
extension VD.SurfaceView: NSMenuItemValidation {
    func validateMenuItem(_ item: NSMenuItem) -> Bool {
        ...
        case #selector(pasteSelection):
            let pb = NSPasteboard.voidSelection
...
extension VD.SurfaceView {
    ...
    /// this to copy styling information as well but we need to augment Void core to
    /// expose that.
    ///
