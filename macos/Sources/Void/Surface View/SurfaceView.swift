...
extension VD {
    ...
    /// Yields a SurfaceView for a void app that can then be used however you want.
    struct SurfaceForApp<Content: View>: View {
        let content: ((SurfaceView) -> Content)

        @StateObject private var surfaceView: SurfaceView

        init(_ app: void_app_t, @ViewBuilder content: @escaping ((SurfaceView) -> Content)) {
            ...
            ZStack {
                ...
                GeometryReader { geo in
                    ...
                    SurfaceRepresentable(view: surfaceView, size: geo.size)
                        ...
                        .focusedValue(\.voidSurfacePwd, surfaceView.pwd)
                        .focusedValue(\.voidSurfaceView, surfaceView)
                        .focusedValue(\.voidSurfaceCellSize, surfaceView.cellSize)
                    #if canImport(AppKit)
                    ...
                    if let surfaceSize = surfaceView.surfaceSize {
                        SurfaceResizeOverlay(
                            geoSize: geo.size,
                            size: surfaceSize,
                            overlay: void.config.resizeOverlay,
                            position: void.config.resizeOverlayPosition,
                            duration: void.config.resizeOverlayDuration,
                            focusInstant: surfaceView.focusInstant)

                    }
                }
                .voidSurfaceView(surfaceView)
                ...
                if void.config.secureInputIndication &&
                    secureInput.enabled &&
                    surfaceFocus &&
                    windowFocus {
                    SecureInputOverlay()
                ...
                if void.config.bellFeatures.contains(.border) {
                ...
                if !surfaceView.healthy {
                    Rectangle().fill(void.config.backgroundColor)
                ...
                if isSplit && !isFocusedSurface {
                    ...
                    if overlayOpacity > 0 {
                        Rectangle()
                            .fill(void.config.unfocusedSplitFill)
                            .allowsHitTesting(false)
    ...
    struct SurfaceSearchOverlay: View {
        ...
        var body: some View {
            GeometryReader { geo in
                ...
                .onReceive(NotificationCenter.default.publisher(for: .voidSearchFocus)) { notification in
    ...
    /// A surface is terminology in Void for a terminal surface, or a place where a terminal is actually drawn
    ...
    /// libvoid, usually from the Void configuration.
    struct SurfaceConfiguration {
        ...
        var context: void_surface_context_e = VOID_SURFACE_CONTEXT_WINDOW

        init() {}

        init(from config: void_surface_config_s) {
        ...
        /// Provides a C-compatible void configuration within a closure. The configuration
        /// and all its string pointers are only valid within the closure.
        func withCValue<T>(view: SurfaceView, _ body: (inout void_surface_config_s) throws -> T) rethrows -> T {
            var config = void_surface_config_new()
            config.userdata = Unmanaged.passUnretained(view).toOpaque()
#if os(macOS)
            config.platform_tag = VOID_PLATFORM_MACOS
            config.platform = void_platform_u(macos: void_platform_macos_s(
                nsview: Unmanaged.passUnretained(view).toOpaque()
            ))
            config.scale_factor = NSScreen.main!.backingScaleFactor
#elseif os(iOS)
            config.platform_tag = VOID_PLATFORM_IOS
            config.platform = void_platform_u(ios: void_platform_ios_s(
            ...
            // libvoid APIs when a UIView is attached to a window/scene. TODO.
...
extension FocusedValues {
    ...
    var voidSurfacePwd: String? {
        get { self[FocusedVoidSurfacePwd.self] }
        set { self[FocusedVoidSurfacePwd.self] = newValue }
    }

    struct FocusedVoidSurfacePwd: FocusedValueKey {
        typealias Value = String
    }

    var voidSurfaceCellSize: OSSize? {
        get { self[FocusedVoidSurfaceCellSize.self] }
        set { self[FocusedVoidSurfaceCellSize.self] = newValue }
    }

    struct FocusedVoidSurfaceCellSize: FocusedValueKey {
