...
/// Wraps a Void surface view in an NSScrollView to provide native macOS scrollbar support.
///
/// ## Coordinate System
...
/// - `surfaceView`: The actual Void renderer, positioned to fill the visible rect
class SurfaceScrollView: NSView {
    ...
    init(contentSize: CGSize, surfaceView: Void.SurfaceView) {
        ...
        // so that our primary Void renderer only needs to render the viewport.
        ...
        // Listen for scrollbar updates from Void
        observers.append(NotificationCenter.default.addObserver(
            forName: .voidDidUpdateScrollbar,
            object: surfaceView,
            queue: .main
        ) { [weak self] notification in
