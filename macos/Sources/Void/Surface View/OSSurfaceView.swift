...
extension Void {
    class OSSurfaceView: OSView, ObservableObject {
        ...
        @Published var surfaceSize: void_surface_size_s?
        ...
        /// A message sent from `void_surface_t` when a child process exited
        @Published private(set) var childExitedMessage: ChildExitedMessage?

        var surface: void_surface_t? {
            nil
        }
        ...
        init(id: UUID?, frame: CGRect) {
            ...
            center.addObserver(
                self,
                selector: #selector(voidDidChangeReadonly(_:)),
        ...
        @objc private func voidDidChangeReadonly(_ notification: Foundation.Notification) {
