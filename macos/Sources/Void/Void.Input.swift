...
extension VD {
    ...
    /// because Void input triggers are a superset of what can be represented by a macOS
    ...
    /// handling for Void is handled at a lower level (usually). This function should generally only
    /// be used for things like NSMenu that only support keyboard shortcuts anyways.
    static func keyboardShortcut(for trigger: void_input_trigger_s) -> KeyboardShortcut? {
        let key: KeyEquivalent
        switch trigger.tag {
        case VOID_TRIGGER_PHYSICAL:
    ...
    /// Returns the event modifier flags set for the Void mods enum.
    static func eventModifierFlags(mods: void_input_mods_e) -> NSEvent.ModifierFlags {
        var flags = NSEvent.ModifierFlags(rawValue: 0)
        if mods.rawValue & VOID_MODS_SHIFT.rawValue != 0 { flags.insert(.shift) }
        if mods.rawValue & VOID_MODS_CTRL.rawValue != 0 { flags.insert(.control) }
        if mods.rawValue & VOID_MODS_ALT.rawValue != 0 { flags.insert(.option) }
        if mods.rawValue & VOID_MODS_SUPER.rawValue != 0 { flags.insert(.command) }
        return flags
    }

    /// Translate event modifier flags to a void mods enum.
    static func voidMods(_ flags: NSEvent.ModifierFlags) -> void_input_mods_e {
        var mods: UInt32 = VOID_MODS_NONE.rawValue

        if flags.contains(.shift) { mods |= VOID_MODS_SHIFT.rawValue }
        if flags.contains(.control) { mods |= VOID_MODS_CTRL.rawValue }
        if flags.contains(.option) { mods |= VOID_MODS_ALT.rawValue }
        if flags.contains(.command) { mods |= VOID_MODS_SUPER.rawValue }
        if flags.contains(.capsLock) { mods |= VOID_MODS_CAPS.rawValue }

        // Handle sided input. We can't tell that both are pressed in the
        // Void structure but that's okay -- we don't use that information.
        let rawFlags = flags.rawValue
        if rawFlags & UInt(NX_DEVICERSHIFTKEYMASK) != 0 { mods |= VOID_MODS_SHIFT_RIGHT.rawValue }
        if rawFlags & UInt(NX_DEVICERCTLKEYMASK) != 0 { mods |= VOID_MODS_CTRL_RIGHT.rawValue }
        if rawFlags & UInt(NX_DEVICERALTKEYMASK) != 0 { mods |= VOID_MODS_ALT_RIGHT.rawValue }
        if rawFlags & UInt(NX_DEVICERCMDKEYMASK) != 0 { mods |= VOID_MODS_SUPER_RIGHT.rawValue }

        return void_input_mods_e(mods)
    }

    /// A map from the Void key enum to the keyEquivalent string for shortcuts. Note that
    /// not all void key enum values are represented here because not all of them can be
    /// mapped to a KeyEquivalent.
    static let keyToEquivalent: [void_input_key_e: KeyEquivalent] = [
        // Function keys
        VOID_KEY_ARROW_UP: .upArrow,
        VOID_KEY_ARROW_DOWN: .downArrow,
        VOID_KEY_ARROW_LEFT: .leftArrow,
        VOID_KEY_ARROW_RIGHT: .rightArrow,
        VOID_KEY_HOME: .home,
        VOID_KEY_END: .end,
        VOID_KEY_DELETE: .deleteForward,
        VOID_KEY_PAGE_UP: .pageUp,
        VOID_KEY_PAGE_DOWN: .pageDown,
        VOID_KEY_ESCAPE: .escape,
        VOID_KEY_ENTER: .return,
        VOID_KEY_TAB: .tab,
        VOID_KEY_BACKSPACE: .delete,
        VOID_KEY_SPACE: .space,
    ]
}
...
extension VD.Input {
    ...
    struct BindingFlags: OptionSet, Sendable {
        ...
        static let consumed = BindingFlags(rawValue: VOID_BINDING_FLAGS_CONSUMED.rawValue)
        static let all = BindingFlags(rawValue: VOID_BINDING_FLAGS_ALL.rawValue)
        static let global = BindingFlags(rawValue: VOID_BINDING_FLAGS_GLOBAL.rawValue)
        static let performable = BindingFlags(rawValue: VOID_BINDING_FLAGS_PERFORMABLE.rawValue)

        init(rawValue: UInt32) {
            self.rawValue = rawValue
        }

        init(cFlags: void_binding_flags_e) {
            self.rawValue = cFlags.rawValue
        }

        var cFlags: void_binding_flags_e {
            void_binding_flags_e(rawValue)
        }
    }
}
...
extension VD.Input {
    ...
    struct KeyEvent {
        ...
        init?(cValue: void_input_key_s) {
            // Convert action
            switch cValue.action {
            case VOID_ACTION_PRESS: self.action = .press
            case VOID_ACTION_RELEASE: self.action = .release
            case VOID_ACTION_REPEAT: self.action = .repeat
        ...
        ///
        /// This method safely converts the Swift KeyEntity to a C `void_input_key_s` struct
        ...
        @discardableResult
        func withCValue<T>(execute: (void_input_key_s) -> T) -> T {
            var keyEvent = void_input_key_s()
...
extension VD.Input {
    ...
    enum Action: String, CaseIterable {
        ...
        var cAction: void_input_action_e {
            switch self {
            case .release: VOID_ACTION_RELEASE
            case .press: VOID_ACTION_PRESS
            case .repeat: VOID_ACTION_REPEAT
            }
        }
    }
}
...
extension VD.Input {
    ...
    struct MouseButtonEvent {
        ...
        init?(state: void_input_mouse_state_e, button: void_input_mouse_button_e, mods: void_input_mods_e) {
            // Convert state
            switch state {
            case VOID_MOUSE_RELEASE: self.action = .release
            case VOID_MOUSE_PRESS: self.action = .press
            default: return nil
            }

            // Convert button
            switch button {
            case VOID_MOUSE_UNKNOWN: self.button = .unknown
            case VOID_MOUSE_LEFT: self.button = .left
            case VOID_MOUSE_RIGHT: self.button = .right
            case VOID_MOUSE_MIDDLE: self.button = .middle
            default: return nil
            }
...
extension VD.Input {
    ...
    enum MouseState: String, CaseIterable {
        ...
        var cMouseState: void_input_mouse_state_e {
            switch self {
            case .release: VOID_MOUSE_RELEASE
            case .press: VOID_MOUSE_PRESS
            }
        }
    }
}
...
extension VD.Input {
    ...
    enum MouseButton: String, CaseIterable {
        ...
        var cMouseButton: void_input_mouse_button_e {
            switch self {
            case .unknown: VOID_MOUSE_UNKNOWN
            case .left: VOID_MOUSE_LEFT
            case .right: VOID_MOUSE_RIGHT
            case .middle: VOID_MOUSE_MIDDLE
            case .four: VOID_MOUSE_FOUR
            case .five: VOID_MOUSE_FIVE
            case .six: VOID_MOUSE_SIX
            case .seven: VOID_MOUSE_SEVEN
            case .eight: VOID_MOUSE_EIGHT
            case .nine: VOID_MOUSE_NINE
            case .ten: VOID_MOUSE_TEN
            case .eleven: VOID_MOUSE_ELEVEN
            }
        }
...
extension VD.Input {
    ...
    struct ScrollMods {
        ...
        var cScrollMods: void_input_scroll_mods_t {
            rawValue
        }
    }
}
...
extension VD.Input {
    ...
    enum Momentum: UInt8, CaseIterable {
        ...
        var cMomentum: void_input_mouse_momentum_e {
            switch self {
            case .none: VOID_MOUSE_MOMENTUM_NONE
            case .began: VOID_MOUSE_MOMENTUM_BEGAN
            case .stationary: VOID_MOUSE_MOMENTUM_STATIONARY
            case .changed: VOID_MOUSE_MOMENTUM_CHANGED
            case .ended: VOID_MOUSE_MOMENTUM_ENDED
            case .cancelled: VOID_MOUSE_MOMENTUM_CANCELLED
            case .mayBegin: VOID_MOUSE_MOMENTUM_MAY_BEGIN
            }
        }
    }
}
...
extension VD.Input {
    ...
    struct Mods: OptionSet {
        ...
        static let none = Mods(rawValue: VOID_MODS_NONE.rawValue)
        static let shift = Mods(rawValue: VOID_MODS_SHIFT.rawValue)
        static let ctrl = Mods(rawValue: VOID_MODS_CTRL.rawValue)
        static let alt = Mods(rawValue: VOID_MODS_ALT.rawValue)
        static let `super` = Mods(rawValue: VOID_MODS_SUPER.rawValue)
        static let caps = Mods(rawValue: VOID_MODS_CAPS.rawValue)
        static let shiftRight = Mods(rawValue: VOID_MODS_SHIFT_RIGHT.rawValue)
        static let ctrlRight = Mods(rawValue: VOID_MODS_CTRL_RIGHT.rawValue)
        static let altRight = Mods(rawValue: VOID_MODS_ALT_RIGHT.rawValue)
        static let superRight = Mods(rawValue: VOID_MODS_SUPER_RIGHT.rawValue)

        var cMods: void_input_mods_e {
            void_input_mods_e(rawValue)
        }
...
extension VD.Input {
    ...
    enum Key: String {
        ...
        var cKey: void_input_key_e {
            switch self {
            // Writing System Keys
            case .backquote: VOID_KEY_BACKQUOTE
            case .backslash: VOID_KEY_BACKSLASH
            case .bracketLeft: VOID_KEY_BRACKET_LEFT
            case .bracketRight: VOID_KEY_BRACKET_RIGHT
            case .comma: VOID_KEY_COMMA
            case .digit0: VOID_KEY_DIGIT_0
            case .digit1: VOID_KEY_DIGIT_1
            case .digit2: VOID_KEY_DIGIT_2
            case .digit3: VOID_KEY_DIGIT_3
            case .digit4: VOID_KEY_DIGIT_4
            case .digit5: VOID_KEY_DIGIT_5
            case .digit6: VOID_KEY_DIGIT_6
            case .digit7: VOID_KEY_DIGIT_7
            case .digit8: VOID_KEY_DIGIT_8
            case .digit9: VOID_KEY_DIGIT_9
            case .equal: VOID_KEY_EQUAL
            case .intlBackslash: VOID_KEY_INTL_BACKSLASH
            case .intlRo: VOID_KEY_INTL_RO
            case .intlYen: VOID_KEY_INTL_YEN
            case .a: VOID_KEY_A
            case .b: VOID_KEY_B
            case .c: VOID_KEY_C
            case .d: VOID_KEY_D
            ...
            case .y: VOID_KEY_Y
            case .z: VOID_KEY_Z
            case .minus: VOID_KEY_MINUS
            case .period: VOID_KEY_PERIOD
            case .quote: VOID_KEY_QUOTE
            case .semicolon: VOID_KEY_SEMICOLON
            case .slash: VOID_KEY_SLASH

            // Functional Keys
            case .altLeft: VOID_KEY_ALT_LEFT
            case .altRight: VOID_KEY_ALT_RIGHT
            case .backspace: VOID_KEY_BACKSPACE
            case .capsLock: VOID_KEY_CAPS_LOCK
            case .contextMenu: VOID_KEY_CONTEXT_MENU
            case .controlLeft: VOID_KEY_CONTROL_LEFT
            case .controlRight: VOID_KEY_CONTROL_RIGHT
            case .enter: VOID_KEY_ENTER
            case .metaLeft: VOID_KEY_META_LEFT
            case .metaRight: VOID_KEY_META_RIGHT
            case .shiftLeft: VOID_KEY_SHIFT_LEFT
            case .shiftRight: VOID_KEY_SHIFT_RIGHT
            case .space: VOID_KEY_SPACE
            case .tab: VOID_KEY_TAB
            case .convert: VOID_KEY_CONVERT
            case .kanaMode: VOID_KEY_KANA_MODE
            case .nonConvert: VOID_KEY_NON_CONVERT

            // Control Pad Section
            case .delete: VOID_KEY_DELETE
            case .end: VOID_KEY_END
            case .help: VOID_KEY_HELP
            case .home: VOID_KEY_HOME
            case .insert: VOID_KEY_INSERT
            case .pageDown: VOID_KEY_PAGE_DOWN
            case .pageUp: VOID_KEY_PAGE_UP

            // Arrow Pad Section
            case .arrowDown: VOID_KEY_ARROW_DOWN
            case .arrowLeft: VOID_KEY_ARROW_LEFT
            case .arrowRight: VOID_KEY_ARROW_RIGHT
            case .arrowUp: VOID_KEY_ARROW_UP

            // Numpad Section
            case .numLock: VOID_KEY_NUM_LOCK
            case .numpad0: VOID_KEY_NUMPAD_0
            case .numpad1: VOID_KEY_NUMPAD_1
            case .numpad2: VOID_KEY_NUMPAD_2
            case .numpad3: VOID_KEY_NUMPAD_3
            case .numpad4: VOID_KEY_NUMPAD_4
            case .numpad5: VOID_KEY_NUMPAD_5
            case .numpad6: VOID_KEY_NUMPAD_6
            case .numpad7: VOID_KEY_NUMPAD_7
            case .numpad8: VOID_KEY_NUMPAD_8
            case .numpad9: VOID_KEY_NUMPAD_9
            case .numpadAdd: VOID_KEY_NUMPAD_ADD
            case .numpadBackspace: VOID_KEY_NUMPAD_BACKSPACE
            case .numpadClear: VOID_KEY_NUMPAD_CLEAR
            case .numpadClearEntry: VOID_KEY_NUMPAD_CLEAR_ENTRY
            case .numpadComma: VOID_KEY_NUMPAD_COMMA
            case .numpadDecimal: VOID_KEY_NUMPAD_DECIMAL
            case .numpadDivide: VOID_KEY_NUMPAD_DIVIDE
            case .numpadEnter: VOID_KEY_NUMPAD_ENTER
            case .numpadEqual: VOID_KEY_NUMPAD_EQUAL
            case .numpadMemoryAdd: VOID_KEY_NUMPAD_MEMORY_ADD
            case .numpadMemoryClear: VOID_KEY_NUMPAD_MEMORY_CLEAR
            case .numpadMemoryRecall: VOID_KEY_NUMPAD_MEMORY_RECALL
            case .numpadMemoryStore: VOID_KEY_NUMPAD_MEMORY_STORE
            case .numpadMemorySubtract: VOID_KEY_NUMPAD_MEMORY_SUBTRACT
            case .numpadMultiply: VOID_KEY_NUMPAD_MULTIPLY
            case .numpadParenLeft: VOID_KEY_NUMPAD_PAREN_LEFT
            case .numpadParenRight: VOID_KEY_NUMPAD_PAREN_RIGHT
            case .numpadSubtract: VOID_KEY_NUMPAD_SUBTRACT
            case .numpadSeparator: VOID_KEY_NUMPAD_SEPARATOR
            case .numpadUp: VOID_KEY_NUMPAD_UP
            case .numpadDown: VOID_KEY_NUMPAD_DOWN
            case .numpadRight: VOID_KEY_NUMPAD_RIGHT
            case .numpadLeft: VOID_KEY_NUMPAD_LEFT
            case .numpadBegin: VOID_KEY_NUMPAD_BEGIN
            case .numpadHome: VOID_KEY_NUMPAD_HOME
            case .numpadEnd: VOID_KEY_NUMPAD_END
            case .numpadInsert: VOID_KEY_NUMPAD_INSERT
            case .numpadDelete: VOID_KEY_NUMPAD_DELETE
            case .numpadPageUp: VOID_KEY_NUMPAD_PAGE_UP
            case .numpadPageDown: VOID_KEY_NUMPAD_PAGE_DOWN

            // Function Section
            case .escape: VOID_KEY_ESCAPE
            case .f1: VOID_KEY_F1
            case .f2: VOID_KEY_F2
            case .f3: VOID_KEY_F3
            case .f4: VOID_KEY_F4
            ...
            case .f8: VOID_KEY_F8
            case .f9: VOID_KEY_F9
            case .f10: VOID_KEY_F10
            case .f11: VOID_KEY_F11
            case .f12: VOID_KEY_F12
            case .f13: VOID_KEY_F13
            case .f14: VOID_KEY_F14
            case .f15: VOID_KEY_F15
            case .f16: VOID_KEY_F16
            case .f17: VOID_KEY_F17
            case .f18: VOID_KEY_F18
            case .f19: VOID_KEY_F19
            case .f20: VOID_KEY_F20
            case .f21: VOID_KEY_F21
            case .f22: VOID_KEY_F22
            case .f23: VOID_KEY_F23
            case .f24: VOID_KEY_F24
            case .f25: VOID_KEY_F25
            case .fn: VOID_KEY_FN
            case .fnLock: VOID_KEY_FN_LOCK
            case .printScreen: VOID_KEY_PRINT_SCREEN
            case .scrollLock: VOID_KEY_SCROLL_LOCK
            case .pause: VOID_KEY_PAUSE

            // Media Keys
            case .browserBack: VOID_KEY_BROWSER_BACK
            case .browserFavorites: VOID_KEY_BROWSER_FAVORITES
            case .browserForward: VOID_KEY_BROWSER_FORWARD
            case .browserHome: VOID_KEY_BROWSER_HOME
            case .browserRefresh: VOID_KEY_BROWSER_REFRESH
            case .browserSearch: VOID_KEY_BROWSER_SEARCH
            case .browserStop: VOID_KEY_BROWSER_STOP
            case .eject: VOID_KEY_EJECT
            case .launchApp1: VOID_KEY_LAUNCH_APP_1
            case .launchApp2: VOID_KEY_LAUNCH_APP_2
            case .launchMail: VOID_KEY_LAUNCH_MAIL
            case .mediaPlayPause: VOID_KEY_MEDIA_PLAY_PAUSE
            case .mediaSelect: VOID_KEY_MEDIA_SELECT
            case .mediaStop: VOID_KEY_MEDIA_STOP
            case .mediaTrackNext: VOID_KEY_MEDIA_TRACK_NEXT
            case .mediaTrackPrevious: VOID_KEY_MEDIA_TRACK_PREVIOUS
            case .power: VOID_KEY_POWER
            case .sleep: VOID_KEY_SLEEP
            case .audioVolumeDown: VOID_KEY_AUDIO_VOLUME_DOWN
            case .audioVolumeMute: VOID_KEY_AUDIO_VOLUME_MUTE
            case .audioVolumeUp: VOID_KEY_AUDIO_VOLUME_UP
            case .wakeUp: VOID_KEY_WAKE_UP

            // Legacy, Non-standard, and Special Keys
            case .copy: VOID_KEY_COPY
            case .cut: VOID_KEY_CUT
            case .paste: VOID_KEY_PASTE
            }
        }
