...
protocol VoidAppDelegate: AnyObject {
    #if os(macOS)
...
extension VD {
    ...
    class App: ObservableObject {
        ...
        /// Optional delegate
        weak var delegate: VoidAppDelegate?
        ...
        /// The void app instance. We only have one of these for the entire app, although I guess
        /// in theory you can have multiple... I don't know why you would...
        @Published var app: void_app_t? {
            didSet {
        ...
        var needsConfirmQuit: Bool {
            ...
            return void_app_needs_confirm_quit(app)
        }
        ...
        init(configPath: String? = nil) {
            ...
            // Create our "runtime" config. The "runtime" is the configuration that void
            // uses to interface with the application runtime environment.
            var runtime_cfg = void_runtime_config_s(
            ...
            // Create the void app.
            guard let app = void_app_new(&runtime_cfg, config.config) else {
                logger.critical("void_app_new failed")
                readiness = .error
                return
            }
            self.app = app
            ...
            void_app_set_focus(app, NSApp.isActive)
        ...
        static func openConfig() {
            ...
            var action = void_action_open_url_s()
            action.kind = VOID_ACTION_OPEN_URL_KIND_TEXT
        ...
        func reloadConfig(soft: Bool = false) {
            ...
            if soft {
                void_app_update_config(app, config.config!)
                return
            }
            ...
            void_app_update_config(app, newConfig.config!)
            /// applied config will be updated in ``Self.configChange(_:target:v:)``
        }

        func reloadConfig(surface: void_surface_t, soft: Bool = false) {
            // Soft updates just call with our existing config
            if soft {
                void_surface_update_config(surface, config.config!)
                return
            }
            ...
            void_surface_update_config(surface, newConfig.config!)
        }
        ...
        func requestClose(surface: void_surface_t) {
            void_surface_request_close(surface)
        }

        func newTab(surface: void_surface_t) {
            let action = "new_tab"
            if !void_surface_binding_action(surface, action, UInt(action.lengthOfBytes(using: .utf8))) {
                logger.warning("action failed action=\(action)")
            }
        }

        func newWindow(surface: void_surface_t) {
        ...
        func split(surface: void_surface_t, direction: void_action_split_direction_e) {
            void_surface_split(surface, direction)
        }

        func splitMoveFocus(surface: void_surface_t, direction: SplitFocusDirection) {
            void_surface_split_focus(surface, direction.toNative())
        }

        func splitResize(surface: void_surface_t, direction: SplitResizeDirection, amount: UInt16) {
            void_surface_split_resize(surface, direction.toNative(), amount)
        }

        func splitEqualize(surface: void_surface_t) {
            void_surface_split_equalize(surface)
        }

        func splitToggleZoom(surface: void_surface_t) {
        ...
        func toggleFullscreen(surface: void_surface_t) {
        ...
        func changeFontSize(surface: void_surface_t, _ change: FontSizeModification) {
            let action: String
            switch change {
        ...
        func toggleTerminalInspector(surface: void_surface_t) {
        ...
        func resetTerminal(surface: void_surface_t) {
            let action = "reset"
            if !void_surface_binding_action(surface, action, UInt(action.lengthOfBytes(using: .utf8))) {
                logger.warning("action failed action=\(action)")
            }
        ...
        #if os(iOS)
        // MARK: Void Callbacks (iOS)

        static func wakeup(_ userdata: UnsafeMutableRawPointer?) {}
        static func action(_ app: void_app_t, target: void_target_s, action: void_action_s) -> Bool { return false }
        static func readClipboard(
            _ userdata: UnsafeMutableRawPointer?,
            location: void_clipboard_e,
        ...
        static func confirmReadClipboard(
            ...
            state: UnsafeMutableRawPointer?,
            request: void_clipboard_request_e
        ) {}

        static func writeClipboard(
            _ userdata: UnsafeMutableRawPointer?,
            location: void_clipboard_e,
            content: UnsafePointer<void_clipboard_content_s>?,
            len: Int,
            confirm: Bool
        ) {}
        ...
        // Called when the selected keyboard changes. We have to notify Void so that
        // it can reload the keyboard mapping for input.
        @objc private func keyboardSelectionDidChange(notification: NSNotification) {
            guard let app = self.app else { return }
            void_app_keyboard_changed(app)
        }

        // Called when the app becomes active.
        @objc private func applicationDidBecomeActive(notification: NSNotification) {
            guard let app = self.app else { return }
            void_app_set_focus(app, true)
        }

        // Called when the app becomes inactive.
        @objc private func applicationDidResignActive(notification: NSNotification) {
            guard let app = self.app else { return }
            void_app_set_focus(app, false)
        }

        // MARK: Void Callbacks (macOS)

        static func closeSurface(_ userdata: UnsafeMutableRawPointer?, processAlive: Bool) {
            let surface = self.surfaceUserdata(from: userdata)
            NotificationCenter.default.post(name: Notification.voidCloseSurface, object: surface, userInfo: [
        ...
        static func readClipboard(
            ...
            // Get our pasteboard
            guard let pasteboard = NSPasteboard.void(location) else { return false }
        ...
        static func completeClipboardRequest(
            _ surface: void_surface_t,
            data: String,
            state: UnsafeMutableRawPointer?,
            confirmed: Bool = false
        ) {
            data.withCString { ptr in
                void_surface_complete_clipboard_request(surface, ptr, state, confirmed)
            }
        }
        ...
        static func writeClipboard(
            ...
            let surface = self.surfaceUserdata(from: userdata)
            guard let pasteboard = NSPasteboard.void(location) else { return }
        ...
        /// Determine if a given notification should be presented to the user when Void is running in the foreground.
        ...
        /// Returns the VoidState from the given userdata value.
        static private func appState(fromView view: SurfaceView) -> App? {
            guard let surface = view.surface else { return nil }
            guard let app = void_surface_app(surface) else { return nil }
            guard let app_ud = void_app_userdata(app) else { return nil }
        ...
        static private func surfaceView(from surface: void_surface_t) -> SurfaceView? {
            guard let surface_ud = void_surface_userdata(surface) else { return nil }
            return Unmanaged<SurfaceView>.fromOpaque(surface_ud).takeUnretainedValue()
        }

        // MARK: Actions (macOS)

        static func action(_ app: void_app_t, target: void_target_s, action: void_action_s) -> Bool {
            // Make sure it a target we understand so all our action handlers can assert
            switch target.tag {
            case VOID_TARGET_APP, VOID_TARGET_SURFACE:
                break
        ...
        private static func quit(_ app: void_app_t) {
        ...
        private static func openURL(
            ...
            switch action.kind {
            case .text:
                // Open with the default editor for `*.void` file or just system text editor
        ...
        private static func undo(_ app: void_app_t, target: void_target_s) -> Bool {
        ...
        private static func redo(_ app: void_app_t, target: void_target_s) -> Bool {
            let undoManager: UndoManager?
            switch target.tag {
            case VOID_TARGET_APP:
                undoManager = (NSApp.delegate as? AppDelegate)?.undoManager
        ...
        private static func newWindow(_ app: void_app_t, target: void_target_s) {
            switch target.tag {
            case VOID_TARGET_APP:
                NotificationCenter.default.post(
                    name: Notification.voidNewWindow,
                    object: nil,
                    userInfo: [:]
                )
            ...
            case VOID_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    ...
                    object: surfaceView,
                    userInfo: [
                        Notification.NewSurfaceConfigKey: SurfaceConfiguration(from: void_surface_inherited_config(surface, VOID_SURFACE_CONTEXT_WINDOW)),
                    ]
                )
        ...
        private static func newTab(_ app: void_app_t, target: void_target_s) {
            switch target.tag {
            case VOID_TARGET_APP:
                NotificationCenter.default.post(
                    name: Notification.voidNewTab,
                    object: nil,
                    userInfo: [:]
                )
            ...
            case VOID_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    ...
                    object: surfaceView,
                    userInfo: [
                        Notification.NewSurfaceConfigKey: SurfaceConfiguration(from: void_surface_inherited_config(surface, VOID_SURFACE_CONTEXT_TAB)),
                    ]
                )
        ...
        private static func newSplit(
            ...
            direction: void_action_split_direction_e) {
            switch target.tag {
            case VOID_TARGET_APP:
            ...
            case VOID_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    name: Notification.voidNewSplit,
                    object: surfaceView,
                    userInfo: [
                        "direction": direction,
                        Notification.NewSurfaceConfigKey: SurfaceConfiguration(from: void_surface_inherited_config(surface, VOID_SURFACE_CONTEXT_SPLIT)),
                    ]
                )
        ...
        private static func presentTerminal(
            ...
            case VOID_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    name: Notification.voidPresentTerminal,
                    object: surfaceView
                )
                return true
        ...
        private static func closeTab(_ app: void_app_t, target: void_target_s, mode: void_action_close_tab_mode_e) {
            ...
            case VOID_TARGET_SURFACE:
                ...
                switch mode {
                case VOID_ACTION_CLOSE_TAB_MODE_THIS:
                    NotificationCenter.default.post(
                        name: .voidCloseTab,
                        object: surfaceView
                    )
                ...
                case VOID_ACTION_CLOSE_TAB_MODE_OTHER:
                    NotificationCenter.default.post(
                        name: .voidCloseOtherTabs,
                        object: surfaceView
                    )
                    return

                case VOID_ACTION_CLOSE_TAB_MODE_RIGHT:
                    NotificationCenter.default.post(
                        name: .voidCloseTabsOnTheRight,
                        object: surfaceView
                    )
                    return
        ...
        private static func closeAllWindows(_ app: void_app_t, target: void_target_s) {
        ...
        private static func toggleFullscreen(
            ...
            case VOID_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    name: Notification.voidToggleFullscreen,
                    object: surfaceView,
                    userInfo: [
        ...
        private static func toggleCommandPalette(
            ...
            case VOID_TARGET_SURFACE:
                ...
                guard let surfaceView = self.surfaceView(from: surface) else { return }
                NotificationCenter.default.post(
                    name: .voidCommandPaletteDidToggle,
                    object: surfaceView
                )
        ...
        private static func setReadonly(
            ...
            case VOID_TARGET_SURFACE:
                ...
                guard let surfaceView = self.surfaceView(from: surface) else { return }
                NotificationCenter.default.post(
                    name: .voidDidChangeReadonly,
                    object: surfaceView,
                    userInfo: [
                        SwiftUI.Notification.Name.ReadonlyKey: v == VOID_READONLY_ON,
                    ]
                )
            ...
            move: void_action_move_tab_s) -> Bool {
                ...
                case VOID_TARGET_SURFACE:
                    ...
                    NotificationCenter.default.post(
                        ...
                        object: surfaceView,
                        userInfo: [
                            SwiftUI.Notification.Name.VoidMoveTabKey: Action.MoveTab(c: move),
                        ]
                    )
        ...
        private static func gotoSplit(
            ...
            direction: void_action_goto_split_e) -> Bool {
                ...
                case VOID_TARGET_SURFACE:
                    ...
                    NotificationCenter.default.post(
                        name: Notification.voidFocusSplit,
                        object: surfaceView,
                        userInfo: [
        ...
        private static func gotoWindow(
            _ app: void_app_t,
            target: void_target_s,
            direction: void_action_goto_window_e
        ) -> Bool {
            ...
            let step: Int
            switch direction {
            case VOID_GOTO_WINDOW_NEXT:
                step = 1
            case VOID_GOTO_WINDOW_PREVIOUS:
                step = -1
            default:
                return false
            }
        ...
        private static func commandFinished(
            _ app: void_app_t,
            target: void_target_s,
            v: void_action_command_finished_s
        ) {
            switch target.tag {
            case VOID_TARGET_APP:
        ...
        private static func toggleFloatWindow(
            _ app: void_app_t,
            target: void_target_s,
            mode mode_raw: void_action_float_window_e
        ) {
        ...
        private static func toggleSecureInput(
            _ app: void_app_t,
            target: void_target_s,
            mode mode_raw: void_action_secure_input_e
        ) {
        ...
        private static func showChildExited(
            _ app: void_app_t,
            target: void_target_s,
            v: void_surface_message_childexited_s,
        ) -> Bool {
            switch target.tag {
            case VOID_TARGET_SURFACE:
                guard let surface = target.target.surface else { return false }
        ...
        private static func promptTitle(
            _ app: void_app_t,
            target: void_target_s,
            v: void_action_prompt_title_e) -> Bool {
        ...
        private static func setMouseVisibility(
            ...
            case VOID_TARGET_SURFACE:
                ...
                guard let surfaceView = self.surfaceView(from: surface) else { return }
                switch v {
                case VOID_MOUSE_VISIBLE:
                    surfaceView.setCursorVisibility(true)

                case VOID_MOUSE_HIDDEN:
        ...
        private static func configReload(
            _ app: void_app_t,
            target: void_target_s,
            v: void_action_reload_config_s) {
            logger.info("config reload notification")

            guard let app_ud = void_app_userdata(app) else { return }
            let void = Unmanaged<App>.fromOpaque(app_ud).takeUnretainedValue()

            switch target.tag {
            case VOID_TARGET_APP:
                void.reloadConfig(soft: v.soft)
                return

            case VOID_TARGET_SURFACE:
                guard let surface = target.target.surface else { return }
                void.reloadConfig(surface: surface, soft: v.soft)
            ...
            v: void_action_config_change_s) {
                ...
                case VOID_TARGET_APP:
                    ...
                    NotificationCenter.default.post(
                        ...
                        object: nil,
                        userInfo: [
                            SwiftUI.Notification.Name.VoidConfigChangeKey: config,
                        ]
                    )
        ...
        private static func colorChange(
            ...
            change: void_action_color_change_s) {
                ...
                case VOID_TARGET_SURFACE:
                    ...
                    guard let surfaceView = self.surfaceView(from: surface) else { return }
                    NotificationCenter.default.post(
                        name: .voidColorDidChange,
                        object: surfaceView,
                        userInfo: [
                            SwiftUI.Notification.Name.VoidColorChangeKey: Action.ColorChange(c: change)
                        ]
                    )
