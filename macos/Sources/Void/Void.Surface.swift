...
extension VD {
    ...
    /// all over.
    ///
    /// Wraps a `void_surface_t`
    final class Surface: Sendable {
        private let surface: void_surface_t
        ...
        var unsafeCValue: void_surface_t {
            surface
        }

        /// Initialize from the C structure.
        init(cSurface: void_surface_t) {
            self.surface = cSurface
        }

        deinit {
            // deinit is not guaranteed to happen on the main actor and our API
            // calls into libvoid must happen there so we capture the surface
        ...
        func sendKeyEvent(_ event: Input.KeyEvent) {
            event.withCValue { cEvent in
                void_surface_key(surface, cEvent)
            }
        }
        ...
        @MainActor
        func keyIsBinding(_ event: void_input_key_s) -> Input.BindingFlags? {
            var flags = void_binding_flags_e(0)
            guard void_surface_key_is_binding(surface, event, &flags) else { return nil }
            return Input.BindingFlags(cFlags: flags)
        }

        /// See `keyIsBinding(_ event: void_input_key_s)`.
        @MainActor
        ...
        @MainActor
        var ttyName: String? {
            let ttyName = AllocatedString(void_surface_tty_name(surface)).string
