...
struct ColorizedVoidIconTests {
    ...
    @Test func codableRoundTripPreservesIcon() throws {
        ...
        let decoded = try JSONDecoder().decode(ColorizedVoidIcon.self, from: data)
    ...
    @Test func decodingUnsupportedVersionThrowsDataCorrupted() {
        ...
        do {
            _ = try JSONDecoder().decode(ColorizedVoidIcon.self, from: data)
            Issue.record("Expected decode to fail for unsupported version")
        } catch let DecodingError.dataCorrupted(context) {
            #expect(context.debugDescription.contains("Unsupported ColorizedVoidIcon version"))
        } catch {
