...
## 현재 상태: 실행 가능 (2026-04-11 session 2)

**`void_main.hexa` 완성** — 8/8 self-test pass, 100KB arm64 binary.

구현 완료:
- ✅ drawRect screen buffer → Core Text per-cell glyph (HexaTermView)
- ✅ persistent event loop (AppKit poll + PTY poll_read 교차)
- ✅ key input → PTY → VT → screen → drawRect 실시간 파이프라인
- ✅ OSC 0 title → tab bar + window title
- ✅ Cmd+T 좌측 탭 / Cmd+W 탭 닫기 / Cmd+Q 종료
- ✅ single-instance lock (Unix socket)
- ✅ VOID.app 번들 (/Applications 설치)

## next

1. 실제 GUI 인터랙션 검증 (셸 프롬프트 + vim/htop)
2. resize 지원 (window resize → PTY TIOCSWINSZ)
3. scrollback buffer
4. Cmd+1~9 탭 전환
5. 선택 + 복사/붙여넣기 (Cmd+C/V)
...
convergence  shared/convergence/void.json   22건 골화
...
app_bundle   VOID.app → /Applications/VOID.app
```
