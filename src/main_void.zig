//! The main entrypoint for the `void` application. This also serves
//! as the process initialization code for the `libvoid` library.
...
const Void = @import("main_c.zig").Void;
...
pub fn main() !MainReturn {
        ...
        switch (@as(ErrSet, @errorCast(err))) {
            ...
            error.InvalidAction => try stderr.print(
                "Error: unknown CLI action specified. CLI actions are specified with\n" ++
                    "the '+' character.\n\n" ++
                    "All valid CLI actions can be listed with `void +help`\n",
                .{},
            ),
    ...
    if (comptime builtin.mode == .Debug) {
        ...
        std.log.warn("You should only use a debug build for developing Void.", .{});
    ...
    if (comptime build_config.app_runtime == .none) {
        ...
        try stdout.print("Usage: void +<action> [flags]\n\n", .{});
        try stdout.print(
            \\This is the Void helper CLI that accompanies the graphical Void app.
            \\To launch the terminal directly, please launch the graphical app
            \\(i.e. Void.app on macOS). This CLI can be used to perform various
            \\actions such as inspecting the version, listing fonts, etc.
            \\
            \\On macOS, the terminal can also be launched using `open -na Void.app`,
            \\or `open -na Void.app --args --foo=bar --baz=qux` to pass arguments.
            \\
...
fn logFn(
    ...
    //
    //   sudo log stream --level debug --predicate 'subsystem=="com.need-singularity.void"'
    //
...
pub const std_options: std.Options = .{
    ...
    // via VOID_LOG because our debug logs are very expensive to
