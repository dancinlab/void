...
pub fn setupFeatures(
    ...
    if (writer.end > 0) {
        try env.put("VOID_SHELL_FEATURES", buf[0..writer.end]);
    }
}
...
test "setup features" {
    ...
    // Test: all features enabled
    {
        ...
        try testing.expectEqualStrings("cursor:blink,path,ssh-env,ssh-terminfo,sudo,title", env.get("VOID_SHELL_FEATURES").?);
    }
    ...
    // Test: all features disabled
    {
        ...
        try testing.expect(env.get("VOID_SHELL_FEATURES") == null);
    }
    ...
    // Test: mixed features
    {
        ...
        try testing.expectEqualStrings("ssh-env,sudo", env.get("VOID_SHELL_FEATURES").?);
    }
    ...
    // Test: blinking cursor
    {
        ...
        try testing.expectEqualStrings("cursor:blink", env.get("VOID_SHELL_FEATURES").?);
    }
    ...
    // Test: steady cursor
    {
        ...
        try testing.expectEqualStrings("cursor:steady", env.get("VOID_SHELL_FEATURES").?);
    }
}
...
fn setupBash(
    ...
    const script_path = try std.fmt.bufPrint(
        &script_path_buf,
        "{s}/shell-integration/bash/void.bash",
        .{resource_dir},
    );
    if (std.fs.openFileAbsolute(script_path, .{})) |file| {
        ...
        env.remove("VOID_BASH_ENV");
        return null;
    }

    try env.put("VOID_BASH_INJECT", buf[0..inject.end]);
    if (rcfile) |v| {
        try env.put("VOID_BASH_RCFILE", v);
    }
    ...
    if (env.get("HISTFILE") == null) {
        ...
        if (try homedir.home(&home_buf)) |home| {
            ...
            try env.put("VOID_BASH_UNEXPORT_HISTFILE", "1");
        }
    }
...
test "bash" {
    ...
    try testing.expectEqualStrings("1", env.get("VOID_BASH_INJECT").?);

    var path_buf: [std.fs.max_path_bytes]u8 = undefined;
    try testing.expectEqualStrings(
        try std.fmt.bufPrint(&path_buf, "{s}/void.bash", .{res.shell_path}),
        env.get("ENV").?,
    );
}
...
test "bash: inject flags" {
    ...
    // bash --norc
    {
        ...
        try testing.expectEqualStrings("bash --posix", command.?.shell);
        try testing.expectEqualStrings("1 --norc", env.get("VOID_BASH_INJECT").?);
    }
    ...
    // bash --noprofile
    {
        ...
        try testing.expectEqualStrings("bash --posix", command.?.shell);
        try testing.expectEqualStrings("1 --noprofile", env.get("VOID_BASH_INJECT").?);
    }
}
...
test "bash: rcfile" {
    ...
    // bash --rcfile
    {
        ...
        try testing.expectEqualStrings("bash --posix", command.?.shell);
        try testing.expectEqualStrings("profile.sh", env.get("VOID_BASH_RCFILE").?);
    }
...
test "bash: HISTFILE" {
    ...
    // HISTFILE unset
    {
        ...
        try testing.expectEqualStrings("1", env.get("VOID_BASH_UNEXPORT_HISTFILE").?);
    }
    ...
    // HISTFILE set
    {
        ...
        try testing.expect(env.get("VOID_BASH_UNEXPORT_HISTFILE") == null);
    }
}
...
test "bash: ENV" {
    ...
    _ = try setupBash(alloc, .{ .shell = "bash" }, res.path, &env);
    try testing.expectEqualStrings("env.sh", env.get("VOID_BASH_ENV").?);
...
/// It is also saved in the `VOID_SHELL_INTEGRATION_XDG_DIR` variable
...
fn setupXdgDataDirs(
    ...
    try env.put("VOID_SHELL_INTEGRATION_XDG_DIR", integ_path);
...
test "xdg: empty XDG_DATA_DIRS" {
    ...
    try testing.expectEqualStrings(
        ...
        env.get("VOID_SHELL_INTEGRATION_XDG_DIR").?,
    );
    try testing.expectEqualStrings(
...
/// which Nushell will use to load `nushell/vendor/autoload/void.nu`.
///
/// We then add `--execute 'use void ...'` to the nu command line to
...
fn setupNushell(
    ...
    // makes our 'void' module automatically available, even if any
    ...
    // enabled shell features (e.g. `use void sudo`). At the moment,
    // shell features are all runtime-guarded in the nushell script.
    try cmd.appendArg("--execute 'use void *'");
...
test "nushell" {
    ...
    try testing.expectEqualStrings("nu --execute 'use void *'", command.?.shell);
...
test "nushell: unsupported options" {
    ...
    for (cmdlines) |cmdline| {
        ...
        try testing.expect(env.get("VOID_SHELL_INTEGRATION_XDG_DIR") != null);
    }
}
...
fn setupZsh(
    ...
    if (env.get("ZDOTDIR")) |old| {
        try env.put("VOID_ZSH_ZDOTDIR", old);
    }
...
test "zsh" {
    ...
    try testing.expect(env.get("VOID_ZSH_ZDOTDIR") == null);
}
...
test "zsh: ZDOTDIR" {
    ...
    try testing.expectEqualStrings("zsh", command.?.shell);
    try testing.expectEqualStrings(res.shell_path, env.get("ZDOTDIR").?);
    try testing.expectEqualStrings("$HOME/.config/zsh", env.get("VOID_ZSH_ZDOTDIR").?);
}
    ...
    fn init(allocator: Allocator, shell: Shell) !TmpResourcesDir {
        ...
        switch (shell) {
            .bash => try tmp_dir.dir.writeFile(.{
                .sub_path = "shell-integration/bash/void.bash",
                .data = "",
            }),
            else => {},
        }
