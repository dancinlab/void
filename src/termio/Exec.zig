...
const Subprocess = struct {
    ...
    pub fn init(gpa: Allocator, cfg: Config) !Subprocess {
        ...
        if (cfg.resources_dir) |dir| {
            log.info("found Void resources dir: {s}", .{dir});
            try env.put("VOID_RESOURCES_DIR", dir);
        }

        // Set our TERM var. This is a bit complicated because we want to use
        // the void TERM value but we want to only do that if we have
        // void in the TERMINFO database.
        //
        ...
        if (cfg.resources_dir) |base| {
            ...
            if (comptime builtin.target.os.tag.isDarwin()) {
                log.warn("void terminfo not found, using xterm-256color", .{});
        ...
        void_path: {
            ...
            const exe_bin_path = std.fs.selfExePath(&exe_buf) catch |err| {
                log.warn("failed to get void exe path err={}", .{err});
                break :void_path;
            };
            const exe_dir = std.fs.path.dirname(exe_bin_path) orelse break :void_path;
            log.debug("appending void bin to path dir={s}", .{exe_dir});

            // We always set this so that if the shell overwrites the path
            // scripts still have a way to find the Void binary when
            // running in Void.
            try env.put("VOID_BIN_DIR", exe_dir);

            // Append if we have a path. We want to append so that void is
            ...
            if (env.get("PATH")) |path| {
                ...
                while (it.next()) |entry| {
                    if (std.mem.eql(u8, entry, exe_dir)) break :void_path;
                }
        ...
        const shell_command: configpkg.Command = shell: {
            ...
            // Always set up shell features (VOID_SHELL_FEATURES). These are
            ...
            const force: ?shell_integration.Shell = switch (cfg.shell_integration) {
                .none => {
                    ...
                    // opt-in since it results in some Void features not
        ...
        const args: []const [:0]const u8 = execCommand(
            ...
            // Void can launch (and maybe the user can debug this further).
    ...
    /// exec is called; it does NOT run in the main Void process.
