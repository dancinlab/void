...
- Any functions must be updated all the way through from here to
  ...
  in `include/void/vt.h`. Specifically:
  ...
  3. Add an `@export` call in `src/lib_vt.zig` with the
     `void_` prefixed symbol name.
  4. Declare it in the corresponding header under `include/void/vt/`.
- In `include/void/vt.h`, always sort the header contents by:
...
- Structs:
  ...
  - May use the "sized struct" pattern: an `extern struct` with
    ...
    callers use `VOID_INIT_SIZED` from `types.h` to zero-initialize and
    set the size.
