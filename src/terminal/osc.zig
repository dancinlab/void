...
pub const Command = union(Key) {
    ...
    /// well defined, so this is currently ignored by Void at the time
    ...
    pub const ProgressReport = struct {
        pub const State = enum(c_int) {
            ...
            test "void.h Command.ProgressReport.State" {
                if (comptime build_options.artifact == .lib) return error.SkipZigTest;
                try lib.checkVoidHEnum(State, "VOID_PROGRESS_STATE_");
            }
        };
        ...
        // sync with void_action_progress_report_s
