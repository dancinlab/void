...
- (BOOL)performKeyEquivalent:(NSEvent *)event {
    ...
    // Cmd+F toggle search overlay. The main loop already intercepts
    // Cmd+KeyDown before sendEvent, so this branch is a safety net for
    // the case where AppKit dispatches performKeyEquivalent directly
    // (e.g. from within a nested event cycle).
    if (ch == 'f' || ch == 'F') {
            ...
            if ([ev type] == NSEventTypeKeyDown) {
                ...
                if (mods & NSEventModifierFlagCommand) {
                    ...
                    if (raw.length > 0) {
                        ...
                        // Opening resets the query so the first keystroke
                        // starts a fresh search; closing is left
                        // g_scroll_offset intact so the user can keep
                        // viewing the match they landed on.
                        if (ch == 'f') {
                            if (g_search_active) search_close();
                            else                 search_open();
                            if (g_window)
