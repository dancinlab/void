...
// ── Grid layout helpers ──
// compute_grid_dims(n, *rows, *cols): derive the (rows, cols) grid for n
// tabs. n must be in [1,9] — callers are responsible for falling back to
// stacked layout for n == 0 or n >= 10.
...
// Tile inset: gap between tiles (pixels). 2px on each side = 4px total
// gutter between neighbouring tiles so the active-border highlight reads
// cleanly without overlapping the next tile's glyphs.
#define TILE_INSET 2

// compute_tile_rect: return the NSRect for tab idx given n_tabs and the
// full content-view bounds. Tiles are laid out in row-major order (first
// row top-to-bottom is rows=0, cols advance left-to-right). Origin (0,0)
// is the top-left because the view is isFlipped. In grid mode the tab
// bar is hidden so tiles use the full bounds minus a TILE_INSET gutter.
static NSRect compute_tile_rect(int idx, NSRect bounds, int n_tabs) {
    ...
    // Row-major: first `grid_cols` tabs fill the top row, next `grid_cols`
    // the next row, etc. If the tab count doesn't divide evenly (e.g. 5
    // tabs in 2×3), trailing slots in the last row are empty.
    ...
    float x = col * tile_w;
    float y = row * tile_h;
    // Inset each side so neighbouring tiles have a visible gap.
    NSRect r = NSMakeRect(x + TILE_INSET,
                          y + TILE_INSET,
                          tile_w - 2 * TILE_INSET,
                          tile_h - 2 * TILE_INSET);
...
// apply_grid_layout: recompute every tab's tile_rows/tile_cols from the
// current view bounds and the active layout mode, then send TIOCSWINSZ
// to each PTY so the shells SIGWINCH into their new sizes. Called when
// the layout mode toggles, the tab count changes, or the window resizes
// in grid mode. In stacked mode every tab's tile_rows/tile_cols mirrors
// the shared g_term_rows/g_term_cols so a round-trip to grid and back
// keeps dims consistent.
static void apply_grid_layout(NSRect bounds) {
    ...
    for (int t = 0; t < g_num_tabs; t++) {
        ...
        g_tabs[t].tile_cols = new_cols;
        // Clamp the bg cursor so a shrink doesn't leave it off-screen.
...
- (BOOL)performKeyEquivalent:(NSEvent *)event {
    ...
    if (ch == 'g') {
        ...
        g_layout_dirty = 1;
        g_full_redraw = 1;
        g_eff_tab_bar_w_cache = -1;  // force main-loop reflow path
        [self setNeedsDisplay:YES];
        return YES;
    }
            ...
            if ([ev type] == NSEventTypeKeyDown) {
                ...
                if (mods & NSEventModifierFlagCommand) {
                    ...
                    if (raw.length > 0) {
                        ...
                        // Cmd+G → toggle tiling grid layout. Flips
                        // g_layout_mode, marks layout dirty, flags a
                        // full redraw, and invalidates the tab-bar
                        // width cache so the resize path below picks up
                        // the eff_tbw change on this same tick.
                        if (ch == 'g') {
                            if (g_num_tabs >= 1 && g_num_tabs <= 9)
