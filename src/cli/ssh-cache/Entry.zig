...
pub fn parse(line: []const u8) ?Entry {
    ...
    const terminfo_version = iter.next() orelse "xterm-void";
...
test "cache entry parsing valid formats" {
    ...
    const entry = Entry.parse("example.com|1640995200|xterm-void").?;
    try testing.expectEqualStrings("example.com", entry.hostname);
    try testing.expectEqual(@as(i64, 1640995200), entry.timestamp);
    try testing.expectEqualStrings("xterm-void", entry.terminfo_version);
    ...
    const complex_entry = Entry.parse("user@server.example.com|1640995200|xterm-void").?;
    try testing.expectEqualStrings(
...
test "cache entry parsing malformed data resilience" {
    ...
    try testing.expect(
        Entry.parse("host|999999999999999999999999999999999999999999999999|xterm-void") == null,
    );
}
