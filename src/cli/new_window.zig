...
pub const Options = struct {
    ...
    /// If set, open up a new window in a custom instance of Void.
...
/// running instance of Void.
///
/// If the `--class` flag is not set, the `new-window` command will try and
/// connect to a running instance of Void based on what optimizations the
/// Void CLI was compiled with. Otherwise the `new-window` command will try
/// and contact a running Void instance that was configured with the same
/// `class` as was given on the command line.
///
/// All of the arguments after the `+new-window` argument (except for the
/// `--class` flag) will be sent to the remote Void instance and will be
...
/// relative to the current working directory that the `void +new-window`
...
/// directory that `void +new-window` is run from will be passed to Void.
///
...
/// `com.dancinlab.void`. If Void is compiled with debug optimizations,
/// the default application ID will be `com.dancinlab.void-debug`.  The
...
/// or it will be ignored and Void will use the default as defined above.
///
/// On GTK, D-Bus activation must be properly configured. Void does not need
...
/// of Void if it is not already running. See the Void website for
...
///     Void. The class must be a valid GTK application ID.
///
///   * `--command`: The command to be executed in the first surface of the new window.
///
///   * `--working-directory=<directory>`: The working directory to pass to Void.
///
