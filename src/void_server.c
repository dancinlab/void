...
// per section, plus one trailing section for any remainder. Uses the
// full grid dim (not s->rows) so the section layout is stable across
// resizes — a shrunken session still hashes the same section count.
...
// dirty. Called from pump_sessions after each drain, and from the
// top of handle_attach so the ATTACH reply never ships stale hashes.
//
// A section is a contiguous band of VS_SECTION_LINES rows. Only the
// first s->cols columns of each row contain live cells (the rest are
// zero-initialized padding that never gets rendered), so we hash
// exactly SECTION_LINES * cols cells per section. Rows past
// VS_GRID_ROWS are zero-padded so build_attach_body and the client
// always see the same byte sequence — critical for the Merkle equality
// check when the grid dim is not an exact multiple of SECTION_LINES.
