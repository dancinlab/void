...
pub const Flags = packed struct {
    ...
    /// and not just while Void is focused. This may not work on all platforms.
...
pub const Action = union(enum) {
    ...
    ///
    /// Void will not process this combination nor forward it to the child
    ...
    ///
    /// This cannot unbind bindings that were not bound by Void or the user
    ...
    /// You must manually untoggle it or quit Void entirely to disable it.
    ///
    ...
    /// version can be found by running `void +version`.
    ...
    ///     plugin in the plugin list. Void would then need to be restarted
    ...
    /// Void becomes focused. When hiding all windows, focus is yielded
    ...
    /// windows.
    ///
    /// Not every action in Void can be undone or redone. The list
    ...
    /// Crash Void in the desired thread for the focused surface.
    ///
    ...
    pub const WriteScreen = struct {
        ...
        pub fn parse(param: []const u8) !WriteScreen {
            ...
            // also very important for backwards compatibility before Void
...
///
/// This must be kept in sync with include/void.h void_input_trigger_s
pub const Trigger = struct {
    ...
    pub fn parse(input: []const u8) !Trigger {
        ...
        loop: while (rem.len > 0) {
            ...
            // keys with Void 1.1.x. We do this last so its least likely
    ...
    /// The values that are backwards compatible with Void 1.1.x.
    /// Void 1.2+ doesn't support these anymore since we moved to
    /// W3C key codes.
...
// For Void 1.2+ we changed our key names to match the W3C and removed
