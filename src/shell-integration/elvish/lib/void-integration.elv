{
  ...
  # Clean up XDG_DATA_DIRS by removing VOID_SHELL_INTEGRATION_XDG_DIR
  if (and (has-env VOID_SHELL_INTEGRATION_XDG_DIR) (has-env XDG_DATA_DIRS)) {
    set-env XDG_DATA_DIRS (str:replace $E:VOID_SHELL_INTEGRATION_XDG_DIR":" "" $E:XDG_DATA_DIRS)
    unset-env VOID_SHELL_INTEGRATION_XDG_DIR
  }

  # List of enabled shell integration features
  var features = [(str:split ',' $E:VOID_SHELL_FEATURES)]

  # State tracking for semantic prompt sequences
  # Values: 'prompt-start', 'pre-exec', 'post-exec'
  fn set-prompt-state {|new| set-env __void_prompt_state $new }

  fn mark-prompt-start {
    if (not-eq $E:__void_prompt_state 'prompt-start') {
  ...
  fn ssh-integration {|@args|
    ...
    if (has-value $features ssh-terminfo) {
      ...
      if (not-eq $ssh-hostname "") {
        var void = $E:VOID_BIN_DIR/"void"
        var ssh-target = $ssh-user"@"$ssh-hostname

        # Check if terminfo is already cached
        if (bool ?($void +ssh-cache --host=$ssh-target)) {
          set ssh-term = "xterm-void"
        } elif (has-external infocmp) {
          var ssh-terminfo = ((external infocmp) -0 -x xterm-void 2>/dev/null | slurp)

          if (not-eq $ssh-terminfo "") {
            echo "Setting up xterm-void terminfo on "$ssh-hostname"..." >&2

            use os
            var ssh-cpath-dir = (os:temp-dir "void-ssh-"$ssh-user".*")
            ...
            if (bool ?(echo $ssh-terminfo | (external ssh) $@ssh-opts -o ControlMaster=yes -o ControlPath=$ssh-cpath -o ControlPersist=60s $@args '
              ...
              $void +ssh-cache --add=$ssh-target >/dev/null
            } else {
  ...
  if (str:contains $E:VOID_SHELL_FEATURES "cursor") {
  ...
  if (and (has-value $features path) (has-env VOID_BIN_DIR)) {
    if (not (has-value $paths $E:VOID_BIN_DIR)) {
        set paths = [$@paths $E:VOID_BIN_DIR]
    }
  }
  if (and (has-value $features sudo) (not-eq "" $E:TERMINFO) (has-external sudo)) {
    edit:add-var sudo~ $sudo-with-terminfo~
  }
  if (and (str:contains $E:VOID_SHELL_FEATURES ssh-) (has-external ssh)) {
