...
pub fn initShared(
    ...
    const dsymutil: ?std.Build.LazyPath = dsymutil: {
        ...
        dsymutil.addArgs(&.{"-o"});
        const output = dsymutil.addOutputFileArg("libvoid.dSYM");
...
pub fn install(self: *const VoidLib, name: []const u8) void {
    ...
    if (self.pkg_config) |pc| {
        step.dependOn(&b.addInstallFileWithDir(
            pc,
            .prefix,
            "share/pkgconfig/void-internal.pc",
        ).step);
    }
    if (self.pkg_config_static) |pc| {
        step.dependOn(&b.addInstallFileWithDir(
            pc,
            .prefix,
            "share/pkgconfig/void-internal-static.pc",
        ).step);
    }
}

pub fn installHeader(self: *const VoidLib) void {
    const b = self.step.owner;
    const header_install = b.addInstallHeaderFile(
        b.path("include/void.h"),
        "void.h",
    );
...
fn pkgConfigFiles(
    ...
    return .{
        .shared = wf.add("void-internal.pc", b.fmt(
            \\prefix={s}
            ...
            \\Description: Void internal library (not for external use)
            \\Version: {f}
            \\Cflags: -I${{includedir}}
            \\Libs: ${{libdir}}/{s}
            \\Libs.private:
        ...
        .static = wf.add("void-internal-static.pc", b.fmt(
            \\prefix={s}
            \\includedir=${{prefix}}/include
            \\libdir=${{prefix}}/lib
            \\
            \\Name: void-internal-static
            \\URL: https://github.com/ghostty-org/ghostty
            \\Description: Void internal library, static (not for external use)
            \\Version: {f}
            \\Cflags: -I${{includedir}}
            \\Libs: ${{libdir}}/{s}
            \\Libs.private:
