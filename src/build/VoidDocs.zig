//! VoidDocs generates all the on-disk documentation that Void is
...
pub fn init(
    ...
    inline for (manpages) |manpage| {
        ...
        try steps.append(b.allocator, &b.addInstallFile(
            markdown_output,
            "share/void/doc/" ++ manpage.name ++ "." ++ manpage.section ++ ".md",
        ).step);
        ...
        try steps.append(b.allocator, &b.addInstallFile(
            ...
            "share/void/doc/" ++ manpage.name ++ "." ++ manpage.section ++ ".html",
        ).step);
...
pub fn install(self: *const VoidDocs) void {
...
/// app.
pub fn installDummy(self: *const VoidDocs, step: *std.Build.Step) void {
    _ = self;
