...
/// Void exe properties
exe_entrypoint: ExeEntrypoint = .void,
...
/// True when Void is being built as a dependency of another project
...
pub fn init(b: *std.Build, appVersion: []const u8, libVersion: []const u8) !Config {
    ...
    // Detect if Void is a dependency of another project.
    ...
    config.version = if (version_string) |v|
        ...
        // Is void a dependency? If so, skip git detection.
    ...
    // libvoid-vt properties

    const lib_version_string = b.option(
        []const u8,
        "lib-version-string",
        "A specific version string to use for the build of libvoid-vt. " ++
            "If not specified, git will be used. This must be a semantic version.",
    );
    ...
    config.emit_lib_vt = b.option(
        bool,
        "emit-lib-vt",
        "Set defaults for a libvoid-vt-only build (disables xcframework, macOS app, and docs).",
    ) orelse false;
    ...
    config.emit_terminfo = b.option(
        bool,
        "emit-terminfo",
        "Install Void terminfo source file",
    ) orelse switch (target.result.os.tag) {
        .windows => true,
    ...
    config.emit_themes = b.option(
        bool,
        "emit-themes",
        "Install bundled iTerm2-Color-Schemes Void themes",
    ) orelse true;
...
/// Void executable being built. Callers should modify this as needed.
...
// (see for example https://github.com/mitchellh/void/issues/1640).
