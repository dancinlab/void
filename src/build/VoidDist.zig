...
const VoidFrameData = @import("VoidFrameData.zig");
...
pub fn init(b: *std.Build, cfg: *const Config) !VoidDist {
    // The name prefix used for all paths in the archive.
    const name = if (cfg.emit_lib_vt) "libvoid-vt" else "void";
    ...
    if (!cfg.emit_lib_vt) {
        ...
        {
            const framedata = VoidFrameData.distResources(b);
    ...
    // embed the Void version in the tarball
    {
    ...
    // are not needed to build libvoid-vt. This significantly reduces
    ...
    // This is the root Void source dir of the extracted source tarball.
...
/// test libvoid-vt, specified as git pathspec exclude patterns.
...
/// of the source tarball with Void. These aren't committed to the Git
