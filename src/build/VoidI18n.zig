...
const domain = "com.mitchellh.void";
...
pub fn init(b: *std.Build, cfg: *const Config) !VoidI18n {
    _ = cfg;
...
pub fn install(self: *const VoidI18n) void {
...
fn createUpdateStep(b: *std.Build) !*std.Build.Step {
    ...
    inline for (gresource.blueprints) |blp| {
        ...
        // a lot of churn because not everyone has the Void code checked out in
...

    {
        ...
        for (gtk_files.items) |item| {
            ...
            // cause a lot of churn because not everyone has the Void code
    ...
    const nautilus_script_path = "dist/linux/void_nautilus.py";
    ...
    const xgettext_merge = b.addSystemCommand(&.{
        ...
        "--copyright-holder=\"Mitchell Hashimoto, Void contributors\"",
        "-o",
        "-",
    });
