% VOID(5) Version @@VERSION@@ | Void terminal emulator configuration file

# NAME

**void** - Void terminal emulator configuration file

# DESCRIPTION

To configure Void, you must use a configuration file. GUI-based configuration
is on the roadmap but not yet supported. The configuration file must be placed
at `$XDG_CONFIG_HOME/void/config.void`, which defaults to `~/.config/void/config.void`
...
`$HOME/Library/Application Support/com.dancinlab.void/config.void`.** This is the
...
executing the command `void +show-config --default --docs`. Note that this will
...
the public config files of many Void users for examples and inspiration.

## Configuration Errors

If your configuration file has any errors, Void does its best to ignore
...
macOS and Linux (GTK). Void does not treat configuration errors as fatal and
will fall back to default values for erroneous keys.

You can also view the full configuration Void is loading using `void
...
--predicate 'subsystem=="com.dancinlab.void"'`.
