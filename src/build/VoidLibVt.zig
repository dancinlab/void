...
const VoidZig = @import("VoidZig.zig");
...
pub fn initWasm(
    ...
    const output = patch_run.addOutputFileArg("void-vt.wasm");
...
pub const AppleLibs = std.EnumMap(ApplePlatform, VoidLibVt);
...
fn initLib(
    ...
    const lib = b.addLibrary(.{
        .name = if (kind == .static) "void-vt-static" else "void-vt",
        .linkage = linkage,
        .root_module = zig.vt_c,
        .version = zig.version,
    });
    ...
    const dsymutil: ?std.Build.LazyPath = dsymutil: {
        ...
        dsymutil.addArgs(&.{"-o"});
        const output = dsymutil.addOutputFileArg("libvoid-vt.dSYM");
    ...
    // it doesn't change -lvoid-vt into an archive-only reference when
...
fn combineArchives(
    ...
    const output = run.addOutputFileArg("libvoid-vt.a");
...
fn pkgConfigFiles(
    ...
    return .{
        .shared = wf.add("libvoid-vt.pc", b.fmt(
            \\prefix={s}
            ...
            \\Description: Void VT library
            \\Version: {f}
            \\Cflags: -I${{includedir}}
            \\Libs: -L${{libdir}} -lvoid-vt
            \\Libs.private: {s}
            \\Requires.private: {s}
        , .{ b.install_prefix, zig.version, libs_private, requires_private })),
        .static = wf.add("libvoid-vt-static.pc", b.fmt(
            \\prefix={s}
            \\includedir=${{prefix}}/include
            \\libdir=${{prefix}}/lib
            \\
            \\Name: libvoid-vt-static
            \\URL: https://github.com/ghostty-org/ghostty
            \\Description: Void VT library (static)
            \\Version: {f}
            \\Cflags: -I${{includedir}}
            \\Libs: ${{libdir}}/{s}
            \\Libs.private: {s}
...
pub fn xcframework(
    ...
    // for VoidKit (the macOS app library).
    const wf = b.addWriteFiles();
    ...
    return XCFrameworkStep.create(b, .{
        .name = "void-vt",
        .out_path = b.pathJoin(&.{ b.install_prefix, "lib/void-vt.xcframework" }),
...
pub fn install(
    ...
    if (self.pkg_config_static) |pkg_config_static| {
        step.dependOn(&b.addInstallFileWithDir(
            pkg_config_static,
            .prefix,
            "share/pkgconfig/libvoid-vt-static.pc",
        ).step);
    }
}
